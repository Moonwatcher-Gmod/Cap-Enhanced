...
if SERVER then
                    ...
                    if (self.Target:IsPlayer() and not self.Target:HasGodMode()) then
                        ...
                        timer.Create("StarGate.UnParalyze", 4, 1, function()
                            if IsValid(self.Entity) and IsValid(self.Target) then
                                GAMEMODE:SetPlayerSpeed(self.Target, 250, 500)
                            end
                        end)
                    end
                else
    ...
    function SWEP:KillEffect()
        ...
        if (self.Owner:IsPlayer() and self.Owner:GetViewEntity() == self.Owner) then
            spectating = 0
        end
...
if CLIENT then
    ...
    --     local WorldModel = ClientsideModel("models/w_hdevice.mdl")
    --     if (self:IsWeaponVisible()) then
            
    --         -- Specify a good position
    --         local offsetVec = Vector(6, -3.7, -0.2)
    --         local offsetAng = Angle(65, -70, 30)
