...
function SWEP:Holster()
    ...
    if SERVER then
        self.Owner:EmitSound(self.Sounds.Holster, 90)
    end
...
function SWEP:PrimaryAttack()
    ...
    if SERVER then
        self:SVPrimaryAttack(add == 0)
    end

    local e = self.Weapon

    -- Sync the animation with the show
    timer.Simple(add, function()
...
if SERVER then
    ...
    SWEP.Sounds = {
        Shot = {Sound("zat/zat_shot1.mp3"), Sound("zat/zat_shot2.mp3")},
        Deploy = Sound("zat/zat_engage.mp3"),
        Holster = Sound("zat/zat_holster.mp3")
    }

    -- Theses entities can't get dissolved!
    SWEP.ProtectedEntities = {"func_", "prop_door_rotating"}
    ...
    function SWEP:DoShoot()
        ...
        if (p:IsPlayer() and p:GetViewEntity() == p) then
            spectating = 0
        end
        ...
        -- Still nothing?, well do a FindInSphere
        if (#targets == 0 and trace.Hit) then
            targets = ents.FindInSphere(trace.HitPos, 50)

            -- Only allow props with physics
            for k, v in pairs(targets) do
                if (not v:GetPhysicsObject():IsValid()) then
                    targets[k] = nil
                end
            end
        end
        ...
        --################### We hit something
        for k, v in pairs(targets) do
            -- Only valid props which are physically
            if (IsValid(v) and v.ZatMode ~= 3 and not v.ZatIgnore) then
                ...
                if (phys:IsValid()) then
                    ...
                    if ((not v.LastZat or v.LastZat + 15 < CurTime()) and not low_hp) then
                        ...
                        if (IS_PLAYER and not v:HasGodMode()) then
                            ...
                            timer.Create("StarGate.UnParalyze", 4, 1, function()
                                if IsValid(v) then
                        ...
                        if ((not disallow_dissolve[v] and not IS_PLAYER and not self:MapObject(v) and phys:IsMoveable() and size < self.MaxSize and dist < self.DissolveDistance) or class == "prop_ragdoll") then
                            ...
                            if (not already_checked[v] and constraint.HasConstraints(v)) then
                                ...
                                if (entities) then
                                    ...
                                    -- Contraption bigger than 20 props - Don't allow dissolve!
                                    if (#entities > 20) then
                                        allow = false
                                    else
                            ...
                            timer.Simple(desintegration_time + 0.3, function()
                                if (v:IsValid()) then
                                    v:Remove()
                                end
                            end)
    ...
    function SWEP:FindNearEnts(entities, checked, passes)
        ...
        for _, e in pairs(entities) do
            if (not checked[e]) then
                ...
                if (radius < 120) then
                    ...
                    -- Finding entities near that prop in most cases
                    for _, v in pairs(ents.FindInSphere(pos, radius + 50)) do
        ...
        if (#entities < 6 and passes < 4) then return self:FindNearEnts(entities, checked, passes) end -- Whatever comes first - Too much entities or too much passes will break the recursive search
