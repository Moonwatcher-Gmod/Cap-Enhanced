...
SWEP.Purpose		= "Allow use of Ancient technology locked by the ATA Gene."
SWEP.Instructions	= "Left click to give or remove the ATA gene."
		...
		timer.Simple(1.6, function()
			if (IsValid(self) and IsValid(self.Owner)) then
				...
				if (self.Owner:GetNWInt('ATAGene', 0)==1)then
					self.Owner:ChatPrint("Ancient Technology Activation Gene Removed")
					self.Owner:SetNWInt('ATAGene', 0)
				else
				self.Owner:ChatPrint("Ancient Technology Activation Gene Injected")
				self.Owner:SetNWInt('ATAGene', 1)
				end		
			end
		end)
	end
