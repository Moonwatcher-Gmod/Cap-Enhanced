...
function StarGate.Trace:InBox(pos, Min, Max)
    if ((pos.x >= Min.x and pos.x <= Max.x) and (pos.y >= Min.y and pos.y <= Max.y) and (pos.z >= Min.z and pos.z <= Max.z)) then return true end
...
function StarGate.Trace:HitWall(coordinate, pos, norm, mul, Min, Max, len, hit_normal)
    ...
    -- The necessary normal length is shorter than the trace's length (we haven't hit anything before we hit the actual object!)
    if (length <= len) then
        -- Check, if the remaining two coordinates are within the Min/Max range!
        local hit = pos + norm

        if ((coordinate == "x" or hit.x >= Min.x and hit.x <= Max.x) and (coordinate == "y" or hit.y >= Min.y and hit.y <= Max.y) and (coordinate == "z" or hit.z >= Min.z and hit.z <= Max.z)) then
...
function StarGate.Trace:CheckCoordinate(coordinate, pos, norm, Min, Max, len, in_box)
    ...
    local hit_normal = Vector(0, 0, 0)
    hit_normal[coordinate] = 1

    --################# We are inside the bounding box - Trace to one wall!
    if (in_box) then
        local mul = 0

        -- Norm == 0 has been avoided in StarGate.Trace:New
        if (norm[coordinate] > 0) then
        ...
        -- We are below the Minimum and the normal goes up => We can hit
        if (pos[coordinate] < Min[coordinate] and norm[coordinate] > 0) then
        ...
        elseif (pos[coordinate] > Max[coordinate] and norm[coordinate] < 0) then
            --Above Max, norm down
...
function StarGate.Trace:New(start, dir, ignore)
    ...
    for e, _ in pairs(self.Entities) do
        if (not quick_ignore[e]) then
            -- Update dimension data and check, if the ent is valid!
            if (self:GetEntityData(e)) then
                ...
                if (self.Classes and self.Classes[class] and (not self.Classes[class].Condition or self.Classes[class].Condition(e, {start, dir, ignore}, trace, in_box))) then
                    ...
                    else -- this code not working, need something to do @ AlexALX --end
                        if (norm.x ~= 0) then
