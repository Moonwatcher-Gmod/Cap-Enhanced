if SERVER then
    ...
    net.Receive("_sg_convars", function(len, ply)
        ...
        if (table.Count(convars_cache) == 0) then
            build_cache()
        end
    ...
    local function CfgBackup(fil)
        if (not file.IsDir("moonwatcher/cfg/backup", "DATA")) then
            file.CreateDir("moonwatcher/cfg/backup")
        end
        ...
        for i = cb, 1, -1 do
            local ni = i - 1
            file.Delete("moonwatcher/cfg/backup/" .. fil .. i .. ".txt")

            if i ~= 1 and file.Exists("moonwatcher/cfg/backup/" .. fil .. ni .. ".txt", "DATA") then
                -- damn, why there is no rename function? modification time lost =(
                file.Write("moonwatcher/cfg/backup/" .. fil .. i .. ".txt", file.Read("moonwatcher/cfg/backup/" .. fil .. ni .. ".txt", "DATA"))
            end

            if i == 1 and file.Exists("moonwatcher/cfg/" .. fil .. ".txt", "DATA") then
                file.Write("moonwatcher/cfg/backup/" .. fil .. i .. ".txt", file.Read("moonwatcher/cfg/" .. fil .. ".txt", "DATA"))
            end
        end
    end
    ...
    net.Receive("_sg_config", function(len, ply)
        ...
        if (typ == 0 or typ == 3 or typ == 4) then
            if (typ == 3) then
                file.Delete("moonwatcher/custom_config.txt")
            end
            ...
            local ini = INIParser:new("moonwatcher/cfg/config.txt", false, false, false, true)

            if (not ini) then
                StarGate.LoadConfig()
                ini = INIParser:new("moonwatcher/cfg/config.txt", false, false, false, true)
            end

            local custom_config = INIParser:new("moonwatcher/cfg/custom_config.txt", false, false, false, true)
        ...
        elseif (typ == 2) then
            ...
            if (ini) then
                ...
                for k, cat in pairs(cust) do
                    ...
                    if (cat:find("cap_disabled_")) then
                        dis = true
                    end
            ...
            if (table.Count(written) > 0) then
                file.Write("moonwatcher/cfg/custom_config.txt", write)
            else
                file.Delete("moonwatcher/cfg/custom_config.txt")
            end
        ...
        elseif (typ == 5 or typ == 7) then
            ...
            local ini = INIParser:new("moonwatcher/cfg/custom_groups.txt", false, false, true, true)

            if (not ini) then
                StarGate.LoadGroupConfig()
                ini = INIParser:new("moonwatcher/cfg/custom_groups.txt", false, false, true, true)
            end
        ...
        elseif (typ == 6) then
            ...
            if (table.Count(written) > 0) then
                file.Write("moonwatcher/cfg/custom_groups.txt", write)
            else
                file.Delete("moonwatcher/cfg/custom_groups.txt")
            end
        ...
        elseif (typ == 9) then
            ...
            if (table.Count(written) > 0) then
                file.Write("moonwatcher/cfg/convars.txt", write)
            else
                file.Delete("moonwatcher/cfg/convars.txt")
            end
        elseif (typ == 10) then
            CfgBackup("convars")
            file.Delete("moonwatcher/cfg/convars.txt")
        elseif (typ == 11) then
            ...
            if (action == 1) then
                ...
                for k, v in pairs(rmf) do
                    ...
                    if (allow) then
                        file.Delete("moonwatcher/cfg/backup/" .. k)
                    end
                end
            elseif (action == 2) then
                ...
                if (allow and file.Exists("moonwatcher/cfg/backup/" .. fil, "DATA")) then
                    net.Start("_sg_config")
                    net.WriteUInt(5, 8)
                    net.WriteUInt(1, 4)
                    net.WriteString(file.Read("moonwatcher/cfg/backup/" .. fil, "DATA"))
                    net.Send(ply)
                end
            elseif (action == 3) then
                ...
                if (allow and file.Exists("moonwatcher/cfg/backup/" .. fil, "DATA")) then
                    if (fil:find("custom_config")) then
                        file.Write("moonwatcher/cfg/custom_config.txt", file.Read("moonwatcher/cfg/backup/" .. fil, "DATA"))
                        StarGate.LoadConfig()
                    elseif (fil:find("custom_groups")) then
                        file.Write("moonwatcher/cfg/custom_groups.txt", file.Read("moonwatcher/cfg/backup/" .. fil, "DATA"))
                        StarGate.LoadGroupConfig()
                    elseif (fil:find("convars")) then
                        ...
                        file.Write("moonwatcher/cfg/convars.txt", file.Read("moonwatcher/cfg/backup/" .. fil, "DATA"))
                ...
                for i, vf in pairs(vfiles) do
                    local f, d = file.Find("moonwatcher/cfg/backup/" .. vf, "DATA")
    ...
    local function SGSetConvar(convar, value)
        if (LocalPlayer():IsAdmin()) then
            ...
            GAMEMODE:AddNotify("You are not admin!", NOTIFY_ERROR, 5)
    ...
    local function SG_Settings_Open()
        ...
        if (CapPanel and IsValid(CapPanel)) then
            CapPanel:Remove()
        end
    ...
    local function SG_Settings_OpenNet()
        ...
        local limits = {{"stargate_cap_menu_01", "destsmall"}, {"stargate_cap_menu_02", "destmedium"}, {"stargate_cap_menu_03", "destmain"}, {"stargate_cap_menu_04", "ioncannon"}, {"stargate_cap_menu_05", "shiprail"}, {"stargate_cap_menu_06", "statrail"}, {"stargate_cap_menu_08", "launchdrone"}, {"stargate_cap_menu_09", "minidrone"}, {"stargate_cap_menu_10", "asgbeam"}, {"stargate_cap_menu_11", "ag3"}, {"stargate_cap_menu_12", "overloader"}, {"stargate_cap_menu_13", "asuran_beam"}, {"stargate_cap_menu_15", "ori_beam"}, {"Wraith Plasma Cannon","wraith_cannon"}, {"stargate_cap_menu_16", "dakara"}, {"stargate_cap_menu_17", "dirn"}, {"stargate_cap_menu_18", "horizon"}, {"stargate_cap_menu_19", "ori"}, {"stargate_cap_menu_20", "staffstat"}, {"stargate_cap_menu_21", "dispenser"}, {"stargate_cap_menu_22", "destcon"}, {"stargate_cap_menu_23", "applecore"}, {"stargate_cap_menu_24", "lantholo"}, {"stargate_cap_menu_25", "shieldcore"}, {"stargate_cap_menu_26", "sod_obelisk"}, {"stargate_cap_menu_27", "anc_obelisk"}, {"entity_destiny_node", "destiny_node"}, {"MCD", "mcd"}, {"IMCD", "imcd"}, {"stargate_cap_menu_39", "ships"}, {"stargate_cap_menu_40", "iris_comp"}, {"weapon_misc_virus", "agv"}, {"CFD", "call_forwarding_device"}}
        SetupConvars(limits, "CAP_", "_max")
        local sboxlimits = {{"stargate_cap_sbox_01", "anim_ramps"}, {"stargate_cap_sbox_02", "ramp"}, {"stargate_cap_sbox_03", "asuran_zpm_hub"}, {"stargate_cap_menu_07", "ashen_defence"}, {"stargate_cap_sbox_04", "zpmhub"}, {"stargate_cap_sbox_05", "sgc_zpm_hub"}, {"stargate_cap_sbox_06", "bearing"}, {"stargate_cap_sbox_07", "brazier"}, {"stargate_cap_sbox_08", "cap_doors_contr"}, {"stargate_cap_sbox_09", "cap_doors_frame"}, {"stargate_cap_sbox_10", "cap_console"}, {"stool_controlpanel", "control_panel"}, {"stargate_cap_sbox_11", "drone_launcher"}, {"stargate_cap_sbox_12", "floorchevron"}, {"stargate_cap_sbox_13", "stargate_iris"}, {"stargate_cap_sbox_28", "goauld_iris"}, {"stargate_cap_sbox_14", "gravitycontroller"}, {"stargate_cap_sbox_15", "jamming_device"}, {"stargate_cap_sbox_16", "naq_gen_mks"}, {"stargate_cap_menu_14", "naquadah_bomb"}, {"stargate_cap_sbox_17", "staff_weapon_glider"}, {"stargate_cap_sbox_18", "cloaking_generator"}, {"stargate_cap_sbox_19", "mobile_dhd"}, {"stargate_cap_sbox_27", "supergate_dhd"}, {"stargate_cap_sbox_20", "shield_generator"}, {"stargate_cap_sbox_21", "tampered_zpm"}, {"stargate_cap_sbox_22", "tokra_emmiter"}, {"stargate_cap_sbox_23", "tokra_key"}, {"stargate_cap_sbox_24", "tollan_disabler"}, {"stargate_cap_sbox_25", "wraith_harvester"}, {"stargate_cap_sbox_26", "zpm_mk3"}, {"stool_naq_bottle", "naquadah_bottle"}, {"stool_sgcscreen", "sgc_monitor"}, {"stool_sgcscreen_srv", "sgc_server"}}
        SetupConvars(sboxlimits, "sbox_max")
        local allowdialgroup = {{"stargate_menu_01", "stargate_candial_groups_menu"}, {"stargate_menu_02", "stargate_candial_groups_dhd"}, {"stargate_menu_03", "stargate_candial_groups_wire"}}
        SetupConvars(allowdialgroup)
        local sgsettings = {{"stargate_menu_04", "stargate_different_dial_menu"}, {"stargate_menu_05", "stargate_energy_dial"}, {"stargate_menu_06", "stargate_energy_dial_spawner"}, {"stargate_menu_28", "stargate_energy_target"}, {"stargate_menu_21", "stargate_protect", 1}, {"stargate_menu_22", "stargate_protect_spawner", 1}, {"stargate_menu_33", "stargate_atlantis_override"}, {"stargate_menu_34", "stargate_gatespawner_enabled"}, {"stargate_menu_34b", "stargate_gatespawner_protect"}, {"stargate_menu_38", "stargate_model_clipping"}, {"stargate_menu_36", "stargate_physics_clipping"}, {"stargate_menu_41", "stargate_random_address"}, {"Stargates a global per default","stargate_global_default",0} }
        SetupConvars(sgsettings)
        local dhdsettings = {{"stargate_menu_07", "stargate_dhd_protect", 1}, {"stargate_menu_08", "stargate_dhd_protect_spawner", 1}, {"stargate_menu_09", "stargate_dhd_destroyed_energy"}, {"stargate_menu_10", "stargate_dhd_close_incoming"}, {"stargate_menu_31", "stargate_dhd_menu"}, {"stargate_menu_32", "stargate_dhd_letters"}, {"stargate_menu_35", "stargate_dhd_ring"}}
        ...
        PropertySheet.CloseButton.DoClick = function()
            CapPanel:Remove()
        end
        ...
        local function SetCapConvar(typ, id, convar, value)
            ...
            if (typ == "sbox") then
                tbl, cat = sboxlimits, "cap_convars"
            end

            if (not CapConvarsTbl[cat]) then
                CapConvarsTbl[cat] = {}
            end
        ...
        cfgbutton.DoClick = function(btn)
            if (lastWarn and IsValid(lastWarn) and lastWarn.Remove) then
                lastWarn:Remove()
            end
        ...
        CapPanel.CfgEditor = function(self, tbl)
            if (CfgEditorFrame.DProperties) then
                CfgEditorFrame.DProperties:Clear()
            end
        ...
        -- this was best my idea (create own spawnmenu tab), because now i can also use this for menu... @ AlexALX
        local cap_ents = {
            ["ent"] = list.Get("CAP.Entity"),
            ["swep"] = list.Get("CAP.Weapon")
        }
        ...
        local rest_types = {
            ["ent_groups_only"] = "ent",
            ["swep_groups_only"] = "swep",
            ["npc_groups_only"] = "npc",
            ["tool_groups_only"] = "tool"
        }
        ...
        for restk, rest in pairs(rest_types) do
            ...
            RestList.DoDoubleClick = function(self, line, data)
                if (lastLine and IsValid(lastLine) and lastLine.Remove) then
                    lastLine:Remove()
                end
                ...
                for n, val in pairs(value) do
                    if (val == "add_shield") then
                        add_shield = true
                        continue
                    end

                    if (val == "exclude_mod") then
                        exclude = true
                        continue
                    end
                ...
                butt.DoClick = function(self)
                    ...
                    if (val ~= "") then
                        ...
                        if (not CfgFrame.CfgTable[rest .. "_groups_only"]) then
                            CfgFrame.CfgTable[rest .. "_groups_only"] = {}
                        end
            ...
            DRestAdd.DoClick = function(self)
                ...
                butt.DoClick = function(self)
                    ...
                    if (not CfgFrame.CfgTable["cap_disabled_" .. rest]) then
                        CfgFrame.CfgTable["cap_disabled_" .. rest] = {}
                    end
        ...
        CfgFrame.UpdateRest = function(self)
            ...
            for restk, rest in pairs(rest_types) do
                ...
                if (restk:find("cap_disabled_")) then
                    RList, dis = RestListsD[restk], true
                end
        ...
        for i = 1, 2 do
            ...
            if (i == 2) then
                grp_type, grp_lang = "stargate_custom_types", "u"
            end
            ...
            GroupList.DoDoubleClick = function(self, line, data)
                ...
                butt.DoClick = function(self)
                    ...
                    if (nam ~= "") then
                        ...
                        if (not CfgGroupFrame.CfgTable[grp_type]) then
                            CfgGroupFrame.CfgTable[grp_type] = {}
                        end
        ...
        CapPanel.OpenCfgView = function(self, name, data)
            if (CapPanel.lastCfgView and IsValid(CapPanel.lastCfgView) and CapPanel.lastCfgView.Remove) then
                CapPanel.lastCfgView:Remove()
            end
            ...
            if (CapPanel and CapPanel.CfgList) then
                ...
                if (action == 1) then
                    if (CapPanel.lastCfgView and IsValid(CapPanel.lastCfgView)) then
                        ...
                        CapPanel.lastCfgView.TxtField.OnTextChanged = function(self)
                            self:SetText(str)
