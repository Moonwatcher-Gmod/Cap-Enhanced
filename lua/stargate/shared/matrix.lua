if (MMatrix == nil) then
   ...
   --]]
    ...
    setmetatable(MMatrix, {
        __call = function(t, ...) return t:New(unpack(arg)) end
    })
    ...
    function MMatrix:Add(m)
        if (not (self and self.matrix)) then
            MMatrix.__error(0, {1, "MMatrix.Add", "MMatrix", self})

            return
        end

        if (not (m and m.matrix)) then
            MMatrix.__error(0, {2, "MMatrix.Add", "MMatrix", m})
    ...
    function MMatrix:Sub(m)
        if (not MMatrix.__IsMatrix(self)) then
            MMatrix.__error(0, {1, "MMatrix.Sub", "MMatrix", self})

            return
        end

        if (not MMatrix.__IsMatrix(m)) then
            MMatrix.__error(0, {2, "MMatrix.Sub", "MMatrix", m})
    ...
    function MMatrix:Multiply(m)
        ...
        if (MMatrix.__IsMatrix(self)) then
            -- Numerical multiply
            if (type(m) == "number") then
                ...
                return MMatrix:New(self.size[1], self.size[2], unpack(args))
            elseif (type(m) == "Vector") then
                -- Gmod 10 vector does only know 3D vectors
            ...
            elseif (MMatrix.__IsMatrix(m)) then
                -- It's a matrix - Either perform a Matrix or Dyadic product (http://en.wikipedia.org/wiki/Dyadic_product)
    ...
    function MMatrix:Pow(n)
        if (type(n) == "number") then
            -- can only pow quadratic matrices
            if (self.size[1] == self.size[2]) then
                -- M^1 = nothing changed - Return self
                if (n == 1) then
                    return self
                elseif (n == -1) then
                    -- Inver the matrix
                ...
                elseif (n == 0) then
                    ...
                    return MMatrix:New(self.size[1], self.size[2], unpack(args))
                elseif (n > 0 and math.ceil(n) == n) then
                    -- Only possible to pow a matrix by integers
            ...
            return
        elseif (n == "t" or n == "T" or (t ~= nil and n == t) or (T ~= nil and n == T)) then
            -- Transpose matrix - But it's better to use MMatrix:Transpose() instead
    ...
    function MMatrix:Trace()
        if (self.size[1] ~= self.size[2]) then
            MMatrix.__error(-1, "MMatrix.Trace: can only calc the trace of a quadratic matrix")
    ...
    function MMatrix:Invert()
        if (self.size[1] ~= self.size[2]) then
            MMatrix.__error(-1, "MMatrix.Invert: can't invert - matrix not quadratic")
        end
        ...
        if (det ~= 0) then return self:Adjugate() / det end -- It can be inverted
    ...
    function MMatrix:Transpose()
        if (self.size[1] ~= self.size[2]) then
            MMatrix.__error(-1, "MMatrix.Transpose: can only transpose quadratic matrices")
    ...
    function MMatrix:Determinant()
        if (self.size[1] ~= self.size[2]) then
            MMatrix.__error(-1, "MMatrix.Determinant: can only calc the determinant of a quadratic matrix")
    ...
    function MMatrix:Stroke(ll, cc)
        ...
        if (ll and ll ~= 0 and ll <= self.size[1]) then
            ll = 1
        end

        if (cc and cc ~= 0 and cc <= self.size[2]) then
            cc = 1
        end
    ...
    function MMatrix.RotationMatrix(vector, angle)
        ...
        if (MMatrix.__IsMatrix(vector)) then
            if (not (vector.size[1] == 3 and vector.size[2] == 1)) then
                MMatrix.__error(-1, "MMatrix.RotationMatrix: Got invalid MMatrix object. (expected 3x1, got " .. vector.size[1] .. "x" .. vector.size[2] .. ")")
        ...
        if (not a) then
            MMatrix.__error(0, {1, "MMatrix.RotationMatrix", "MMatrix,Vector", vector})
        ...
        return MMatrix:New(3, 3, (c + (1 - c) * a[1] ^ 2), ((1 - c) * a[1] * a[2] - s * a[3]), ((1 - c) * a[1] * a[3] + s * a[2]), ((1 - c) * a[2] * a[1] + s * a[3]), (c + (1 - c) * a[2] ^ 2), ((1 - c) * a[2] * a[3] - s * a[1]), ((1 - c) * a[1] * a[3] - s * a[2]), ((1 - c) * a[3] * a[2] + s * a[1]), (c + (1 - c) * a[3] ^ 2))
    end
