...
net.Receive("StarGate.VGUI.Menu", function(len)
    ...
    -- 0 is normal menu, -1 is alternative (without dial)
    if (type <= 0) then
    ...
    elseif (type == 1) then
        -- 1 is normal dial menu (used in ships/mobile dhd etc)
        ...
        VGUI:SetVisible(true)
    elseif (type == 2) then
        -- 2 is for dial menu with feature to override candialg option (for dhds/destiny console etc).
...
usermessage.Hook("StarGate.DialMenuDHDClose", function(data)
    if (VGUI and VGUI:IsValid()) then
        VGUI:SetVisible(false)
    end
end)
...
hook.Add("GUIMousePressed", "StarGate.DHD.GUIMousePressed_Group", function(_, dir)
    --if(IsValid(DHD)) then
    local p = LocalPlayer()
...
function PANEL:SetSettings(entity, groupsystem, alternatemenu, candialg)
    if (not IsValid(entity) or not entity.IsStargate) then
        self:Remove()
    end
    ...
    self.VGUI = {
        ...
        AddressLabel = {vgui.Create("DLabel", self), vgui.Create("DLabel", self)},
    ...
    if (entity:GetNetworkedInt("SG_BLOCK_ADDRESS") <= 0 or entity:GetNetworkedInt("SG_BLOCK_ADDRESS") == 1 and not entity.IsUniverseGate or self.Class == "stargate_supergate") then
    ...
    elseif (not self.Entity.IsUniverseGate) then
        pos = {103, 142, 130, 160}
    ...
    elseif (self.Entity.IsUniverseGate) then
        pos = {103, 136, 130, 145}
    ...
    if (not self.Alternative) then
        ...
        self.VGUI.AddressSelectLabel = {vgui.Create("DLabel", self), vgui.Create("DLabel", self)} -- Black background/Shadow
    end
    ...
    self.VGUI.AddressPanel.OnAddressSet = function(e, address, group)
        -- Avoids the VGUI sending "SetGateAddress" everytime we open it!
        if ((self.AlphaTime or 0) + 0.3 < CurTime()) then
            if (IsValid(e)) then
            ...
            if (self.VGUI.AddressSelect) then
                ...
                timer.Create("_StarGate.RefreshAddressList", 0.25, 1, function()
    ...
    self.VGUI.NameTextEntry.OnTextChanged = function(TextEntry)
        ...
        timer.Create("_StarGate.SetNameTime", 1, 1, function()
            if (IsValid(e)) then
                e:SetGateName(s)
            end
        end)
...
function PANEL:RegisterHooks()
    ...
    self.Think = function(self)
        ...
        -- Prevents some resnapping bugs
        if (x ~= ScrW() / 2 and y ~= ScrH() and x > 1 and y > 1) then
...
function PANEL:SetSettings(entity, groupsystem, candialg, nox, orlin)
    ...
    self.VGUI = {
        ...
        AddressSelectLabel = {vgui.Create("DLabel", self), vgui.Create("DLabel", self)}
    }
    ...
    self.VGUI.AddressSelect.OnTextChanged = function(AddressSelect)
        if (self.OnTextChanged) then
            self.OnTextChanged(AddressSelect)
        end
    end
...
function PANEL:SetStatus(g, no_message)
    ...
    if (g:len() == 2) then
        ...
        -- SET THE GROUP!
        if (self.OnAddressSet) then
            self.OnAddressSet(self.Entity, g:upper())
        end
...
function PANEL:SetSettings(entity, groupsystem)
    ...
    self.IsSGU = (self.Entity.IsUniverseGate)
...
function PANEL:SetStatus(s, no_message)
    ...
    if (len == 6) then
        ...
        for k, v in pairs(self:GetGates()) do
            ...
            if (address:find(letters[1]) and address:find(letters[2]) and address:find(letters[3]) and address:find(letters[4]) and address:find(letters[5]) and address:find(letters[6])) then
                -- We have entered the same/similar address we had before - So we haven't changed anything
                if (tonumber(v.ent) == self.Entity:EntIndex()) then
                    set = false
                    break
                else
                    valid = false
        ...
        if (valid) then
            ...
            -- SET THE ADDRESS!
            if (self.OnAddressSet) then
                self.OnAddressSet(self.Entity, s:upper())
            end
        else
            -- INVALID!
    ...
    elseif (len == 0) then
        ...
        if (not no_message) then
            self:ShowMessage(SGLanguage.GetMessage("stargate_vgui_adrclr"), "Info")
        end

        -- CLEAR ADDRESS
        if (self.OnAddressSet) then
            self.OnAddressSet(self.Entity, "")
        end
    else
        -- Typing address!
        self.VGUI.StatusImage:SetImage(self.Images.Editing)
    ...
    if (len == 6 and g:len() == 2) then
        ...
        if (valid) then
            for _, v in pairs(self:GetGates()) do
                ...
                if (address:find(letters[1]) and address:find(letters[2]) and address:find(letters[3]) and address:find(letters[4]) and address:find(letters[5]) and address:find(letters[6]) and (g == group or g:sub(1, 1) == group:sub(1, 1))) then
        ...
        if (valid) then
            ...
            -- SET THE ADDRESS!
            if (self.OnAddressSet) then
                self.OnAddressSet(self.Entity, s:upper(), g:upper())
            end
    ...
    if (len == 6 and g:len() == 3) then
        ...
        if (valid) then
            for _, v in pairs(self:GetGates()) do
                ...
                if (address:find(letters[1]) and address:find(letters[2]) and address:find(letters[3]) and address:find(letters[4]) and address:find(letters[5]) and address:find(letters[6]) and group:len() == 3 or not self:IsSharedGroup(group) and g == group) then
                    -- We have entered the same/similar address we had before - So we haven't changed anything
                    if (tonumber(v.ent) == self.Entity:EntIndex()) then
                        if (group:find(g)) then
                            set = false
                        ...
                        if (self.Entity:GetGateGroup() == g) then break end
                    else
                        valid = false
...
local function ApplyGlyphs(self, galaxy, small)
    ...
    if (small) then
        w, wa, wu, h, ha, hu = 128, 137, 78, 18, 18, 23
    end
    ...
    if (IsValid(ent)) then
        block_address = ent:GetNetworkedInt("SG_BLOCK_ADDRESS")
    end

    if (IsValid(ent) and ent.IsUniverseGate) then
    ...
    for i = 1, #mlist do
        ...
        if (not universe and block_address == 1) then
            blocked = false
        end
...
function PANEL:SetSettings(entity, groupsystem, candialg, hidedialmode)
    ...
    self.VGUI.AddressListView.OnRowSelected = function(ListView, Row)
        ...
        if (ListView:GetLine(selected).Blocked) then
            ListView:GetLine(selected):SetSelected(false)
            self.LastSelected = 0
    ...
    if (self.GroupSystem or self.IsSuper) then
        ...
        self.VGUI.AddressMultiChoice.TextEntry.OnTextChanged = function(TextEntry)
            ...
            if (text ~= self.Data.Address) then
                ...
                if (text:sub(8, 8) == "!") then
                    text = text:sub(1, 8)
                elseif (text:sub(7, 7) == "@") then
                    text = text:sub(1, 7)
                else
                    text = text:sub(1, 6)
                end

                -- if can't dial 8-9 chevrons by convars
                if (self.MaxSymbols == 6) then
                    text = text:sub(1, 6)
                end
...
function PANEL:DialGate(address)
    ...
    if (address:len() >= 6) then
        ...
        if (self.OnDial) then
            self.OnDial(self.Entity, address, self.VGUI.DialTypeCheckbox:GetChecked())
        end
    else
    ...
    if (self.GroupAllowed) then
        ...
        for k, v in pairs(self.Gates) do
            ...
            if (tonumber(v.ent) ~= self.Entity:EntIndex() and not v.private) then
                ...
                if (address ~= "" and group ~= "" and IsValid(ent) and (not locale and not ent:GetLocale() and not self.LocalGate or (ent:GetGateGroup() == group or v.sgugate and ent.IsUniverseGate)) and (address ~= ent:GetGateAddress() or group ~= ent:GetGateGroup())) then
                    ...
                    if (ent:GetGateGroup() ~= group and (v.sgugate ~= true or not ent.IsUniverseGate) or c_range > 0 and range > c_range and ent:GetGateGroup():len() == 3) then
                    ...
                    if (not table.HasValue(gates, address) and (not ent:GetNWBool("Chev9Special") or address2:len() == 9) and (not s or (self:Find(address, s) or (name ~= "" and self:Find(name, s))))) then
                        ...
                        if (not ent:CheckEnergy(v, address2:len())) then
                            energy = "false"
                        end
            ...
            if (tonumber(v.ent) ~= self.Entity:EntIndex() and not v.private) then
                ...
                if (address ~= "" and IsValid(ent)) then
                    ...
                    if (not s or (self:Find(address, s) or (name ~= "" and self:Find(name, s)))) then
                        ...
                        if (not ent:CheckEnergy(v, address:len() + 1)) then
                            energy = "false"
                        end
            ...
            if (tonumber(v.ent) ~= self.Entity:EntIndex() and not v.private) then
                ...
                if (address ~= "") then
                    if (IsValid(g)) then
                        ...
                        if (v.galaxy or g:GetGalaxy() or v.sgugate and g.IsUniverseGate and c_range > 0 and range > c_range) then
                            address = address .. "@"
                        end
                        ...
                        if (#address == 7 and g:GetGalaxy() and v.galaxy and ((v.class ~= "stargate_atlantis" and g:GetClass() ~= "stargate_atlantis") and (not v.sgugate and not g.IsUniverseGate))) then
                            address = string.Explode("@", tostring(address))
                            address = address[1]
                        end

                        --#address == 7 and
                        if ((v.sgugate and not g.IsUniverseGate) or (not v.sgugate and g.IsUniverseGate)) then
                            address = string.Explode("@", tostring(address))
                            address = address[1] .. "@!"
                        end
                    end
                    ...
                    if (IsValid(ent) and (not s or (self:Find(address, s) or (name ~= "" and self:Find(name, s))))) then
                        ...
                        if (self.CanDialGroups or address:len() == 6) then
                            ...
                            if (IsValid(g) and v.blocked) then
                                blocked = "true"
                            end
...
function PANEL:IsConcept(ent)
    ...
    for _, v in pairs(self:FindDHD()) do
        if (v:IsValid() and v:GetClass() == "dhd_concept") then return true end
    end
