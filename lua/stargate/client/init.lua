...
function StarGate.CompareVersion(ver, verc, add)
    ...
    if tonumber(v2t[1]) < tonumber(v1t[1]) or tonumber(v2t[1]) == tonumber(v1t[1]) and tonumber(v1t[2]) > tonumber(v2t[2]) then return true end
...
function StarGate.Hook.GetInternetStatus(_, key)
    ...
    if (mode == "1") then
        ...
        http.Fetch(StarGate.HTTP.VER, function(html, size)
...
function StarGate.InstalledOnClient()
    ...
    for _, v in pairs(engine.GetAddons()) do
        if (v.mounted) then
            ...
            if (not cap_installed and tbl[tonumber(v.wsid)]) then
                cap_installed = true
                break
            end
        end
    ...
    if (ws_addonlist[1839605925] or cap_installed) then
        StarGate.InstalledOnCl = true

        return true
    end

    if (table.HasValue(addonlist, "Carter Addon Pack") or table.HasValue(addonlist, "Carter Addon Pack - Resources")) then
        StarGate.InstalledOnCl = true
...
CreateClientConVar("cl_shield_design", 1, true, false)
