...
function StarGate.WireRD.GetWire(self, key, default, out)
    if (WireAddon) then
        if (out) then
            if (self.Outputs and self.Outputs[key] and self.Outputs[key].Value) then return self.Outputs[key].Value or default or WireLib.DT[self.Outputs[key].Type].Zero end
        else
            if (self.Inputs and self.Inputs[key] and self.Inputs[key].Value) then return self.Inputs[key].Value or default or WireLib.DT[self.Inputs[key].Type].Zero end
        end
    end
    -- Error. Either wire is not installed or the input is not SET. Return the default value instead
    ...
    if (self.HasRD) then
        ...
        elseif (StarGate.RDEnv()) then
            if self.node then
                ...
                if self.resources then return self.resources[resource] or default or 0 end
...
function StarGate.WireRD.ConsumeResource(self, resource, ammount)
    if (self.HasRD) then
        ...
        elseif (StarGate.RDEnv()) then
            if self.node then return self.node:ConsumeResource(resource, ammount or 0) end
        elseif (StarGate.RDTwo()) then
    ...
    if (self.HasRD) then
        ...
        elseif (StarGate.RDEnv()) then
            if self.node then
                ...
                if self.maxresources then return self.maxresources[resource] end
            end
...
function StarGate.WireRD.GetEntListTable(ent)
    ...
    if (StarGate.HasResourceDistribution) then
        ...
        elseif (StarGate.RDEnv()) then
            if (ent.node and ent.node.connected) then return ent.node.connected or {} end
        ...
        elseif (StarGate.RDTwo()) then
            ...
            for k, v in pairs(entTable) do
                if IsValid(v) and v.resources2links then return v.resources2links or {} end
            end
...
function StarGate.WireRD.Connected(ent)
    ...
    if (StarGate.HasResourceDistribution) then
        if (StarGate.RDThree()) then
            local entTable = RD.GetEntityTable(ent)
            if (entTable["network"] and entTable["network"] > 0) then return true end
        elseif (StarGate.RDEnv()) then
            if (ent.node and IsValid(ent.node)) then return true end
        elseif (StarGate.RDTwo()) then
            if (ent.resources2links and table.Count(ent.resources2links) > 0) then return true end
        end
    end
...
--  Duplicator handling
--##############################
--add duping for cables
local function Environments_ApplyDupeInfo(ent, CreatedEntities, Player)
    if ent.EntityMods and ent.EntityMods.EnvDupeInfo then
        --stops people from pasting admin only stuff
        if ent.AdminOnly and not Player:IsAdmin() then
            ent:Remove()
...
function StarGate.WireRD.PreEntityCopy(self)
    if (RD_BuildDupeInfo) then
        RD_BuildDupeInfo(self.Entity)
    end

    if (RD and RD.BuildDupeInfo) then
...
function StarGate.WireRD.PostEntityPaste(self, Player, Ent, CreatedEntities)
    if (RD_ApplyDupeInfo) then
        RD_ApplyDupeInfo(Ent, CreatedEntities)
    end

    if (not Environments and RD and RD.ApplyDupeInfo) then
        RD.ApplyDupeInfo(Ent, CreatedEntities)
    end

    if (Environments) then
        Environments_ApplyDupeInfo(Ent, CreatedEntities, Player)
    end
