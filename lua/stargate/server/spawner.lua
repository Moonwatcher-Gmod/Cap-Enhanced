...
function StarGate.GateSpawner.Spawn(v, protect, k, k2)
    ...
    if (v.position and v.classname) then
        ...
        timer.Simple(0.1 * k, function()
            ...
            elseif (IsSGULightUp) then
                -- Weld sgu stuff to nearest gates
                for _, v in pairs(ents.FindInSphere(pos, 200)) do
                    if (IsValid(v) and v.IsStargate and v.IsUniverseGate) then
            ...
            if (v.__id and e:GetClass() == "prop_physics") then
                duplicator.StoreEntityModifier(e, "GateSpawnerProp", {
                    GateSpawner = true,
                    ID = e.GateSpawnerID
                })
...
function StarGate.GateSpawner.InitialSpawn(reload)
    if (not StarGate.GateSpawner.Block) then
        ...
        local remove = {ents.FindByClass("stargate_*"), ents.FindByClass("goauld_iris"), ents.FindByClass("dhd_*"), ents.FindByClass("mobile_dhd"), ents.FindByClass("ring_*"), ents.FindByClass("bearing"), ents.FindByClass("brazier"), ents.FindByClass("floorchevron"), ents.FindByClass("destiny_timer"), ents.FindByClass("destiny_console"), ents.FindByClass("kino_dispenser"), ents.FindByClass("ramp"), ents.FindByClass("ramp_2"), ents.FindByClass("future_ramp"), ents.FindByClass("sgc_ramp"), ents.FindByClass("icarus_ramp"), ents.FindByClass("sgu_ramp"), ents.FindByClass("goauld_ramp"), ents.FindByClass("gravitycontroller"), ents.FindByClass("atlantis_light"), ents.FindByClass("atlantis_transporter"), ents.FindByClass("prop_physics"), ents.FindByClass("cap_doors*"), ents.FindByClass("cap_console")}
    ...
    if (not StarGate.GateSpawner.Spawned or reload) then
        ...
        if (GetConVar("stargate_gatespawner_enabled"):GetBool() and StarGate.GateSpawner.LoadConfig()) then
            ...
            local tbl = {StarGate.GateSpawner.Ramp, StarGate.GateSpawner.Gates, StarGate.GateSpawner.Iris, StarGate.GateSpawner.DHDs, StarGate.GateSpawner.MDHDs, StarGate.GateSpawner.DestinyTimer, StarGate.GateSpawner.DestinyConsole, StarGate.GateSpawner.KinoDispenser, StarGate.GateSpawner.RingBase, StarGate.GateSpawner.RingPanel, StarGate.GateSpawner.Brazier, StarGate.GateSpawner.SGULightUp, StarGate.GateSpawner.GravityController, StarGate.GateSpawner.AtlantisLight, StarGate.GateSpawner.AtlantisTransporter, StarGate.GateSpawner.Props, StarGate.GateSpawner.Doors, StarGate.GateSpawner.DoorButtons, StarGate.GateSpawner.Console}

            for k, t in pairs(tbl) do
                for _, v in pairs(t) do
                    table.insert(StarGate.GateSpawner.Ents, {
                        Entity = StarGate.GateSpawner.Spawn(v, protect, i),
                        SpawnData = v
                    })
    ...
    if (StarGate.GateSpawner.Spawned) then
        ...
        for k, v in pairs(StarGate.GateSpawner.Ents) do
            ...
            if (not v.Entity or not v.Entity:IsValid()) then
                StarGate.GateSpawner.Ents[k] = {
                    Entity = StarGate.GateSpawner.Spawn(v.SpawnData, protect, i, k),
                    SpawnData = v.SpawnData
                }
...
-- ############### Init @aVoN
-- Spawn them, 2 seconds after the map start
timer.Simple(2, function()
    StarGate.GateSpawner.InitialSpawn()
end)

-- Check for existance every 3 seconds
timer.Create("StarGate.GateSpawner.AutoRespawn", 3, 0, function()
    StarGate.GateSpawner.AutoRespawn()
end)
...
function StarGate.GateSpawner.CreateFile(p)
    if (not IsValid(p) or p:IsAdmin()) then
        ...
        for _, v in pairs(ents.FindByClass("stargate_*")) do
            if (v.IsStargate) then
                ...
                if (v.IsGroupStargate and groupsystem) then
                    ...
                    if (v.ChevDestroyed) then
                        f = f .. "chevdestroyed=" .. tostring(v.ChevDestroyed) .. "\n"
                    end
        ...
        for _, v in pairs(ents.FindByClass("gravitycontroller")) do
            --only nodes with SGAPowerNode Type
            if v.ConTable["bSGAPowerNode"][2] == 1 then
        ...
        for _, v in pairs(ents.FindByClass("cap_doors_*")) do
            ...
            if (class == "cap_doors_frame") then
                ...
                if (v:GetMaterial() == "madman07/doors/atlwall_red") then
                    f = f .. "diffmat=true\n"
...
concommand.Add("stargate_gatespawner_reload", function(p)
...
function StarGate.GateSpawner.Restored()
    ...
    timer.Simple(dly, function()
        local check = {ents.FindByClass("stargate_*"), ents.FindByClass("goauld_iris"), ents.FindByClass("dhd_*"), ents.FindByClass("mobile_dhd"), ents.FindByClass("ring_*"), ents.FindByClass("bearing"), ents.FindByClass("brazier"), ents.FindByClass("floorchevron"), ents.FindByClass("destiny_timer"), ents.FindByClass("destiny_console"), ents.FindByClass("kino_dispenser"), ents.FindByClass("ramp"), ents.FindByClass("ramp_2"), ents.FindByClass("future_ramp"), ents.FindByClass("sgc_ramp"), ents.FindByClass("icarus_ramp"), ents.FindByClass("sgu_ramp"), ents.FindByClass("goauld_ramp"), ents.FindByClass("gravitycontroller"), ents.FindByClass("atlantis_light"), ents.FindByClass("atlantis_transporter"), ents.FindByClass("prop_physics"), ents.FindByClass("cap_doors*"), ents.FindByClass("cap_console")}
        local protect = GetConVar("stargate_gatespawner_protect"):GetBool()
