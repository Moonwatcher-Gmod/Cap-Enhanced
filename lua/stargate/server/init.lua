...
function StarGate.LoadGroupConfig()
    if (not file.Exists("moonwatcher/cfg/custom_groups.txt", "DATA") and file.Exists("lua/data/moonwatcher/custom_groups.lua", "GAME")) then
        file.Write("moonwatcher/cfg/custom_groups.txt", file.Read("lua/data/moonwatcher/custom_groups.lua", "GAME"))
    end

    StarGate.CUSTOM_GROUPS = {}
    StarGate.CUSTOM_TYPES = {}
    local ini = INIParser:new("moonwatcher/cfg/custom_groups.txt", false, false, true)
...
function StarGate.LoadConfig(p, cmd, args)
    if (not IsValid(p) or p:IsAdmin() or game.SinglePlayer()) then
        ...
        if (not INIParser) then
            include("ini_parser.lua")
        end

        if (not file.IsDir("moonwatcher/cfg", "DATA")) then
            file.CreateDir("moonwatcher/cfg")

            -- add old configs
            if (file.Exists("moonwatcher/custom_config.txt", "DATA")) then
                file.Write("moonwatcher/cfg/custom_config.txt", file.Read("moonwatcher/custom_config.txt", "DATA"))
                file.Delete("moonwatcher/custom_config.txt")
            end

            if (file.Exists("moonwatcher/custom_groups.txt", "DATA")) then
                file.Write("moonwatcher/cfg/custom_groups.txt", file.Read("moonwatcher/custom_groups.txt", "DATA"))
                file.Delete("moonwatcher/custom_groups.txt")
            end

            if (file.Exists("moonwatcher/convars.txt", "DATA")) then
                file.Write("moonwatcher/cfg/convars.txt", file.Read("moonwatcher/convars.txt", "DATA"))
                file.Delete("moonwatcher/convars.txt")
            end

            file.Delete("moonwatcher/config.txt")
            file.Delete("moonwatcher/how to create your own config.txt")
        end

        if (file.Exists("lua/data/moonwatcher/config.lua", "GAME")) then
            file.Write("moonwatcher/cfg/config.txt", file.Read("lua/data/moonwatcher/config.lua", "GAME"))
        end

        if (file.Exists("lua/data/moonwatcher/how to create your own config.lua", "GAME")) then
            file.Write("moonwatcher/cfg/how to create your own config.txt", file.Read("lua/data/moonwatcher/how to create your own config.lua", "GAME"))
        end

        local ini = INIParser:new("moonwatcher/cfg/config.txt", false)
        local custom_config = INIParser:new("moonwatcher/cfg/custom_config.txt", false)
        ...
        -- fix for reload.
        if (StarGate.Hook.PlayerInitialSpawn) then
            timer.Simple(0, function()
                StarGate.Hook.PlayerInitialSpawn(NULL, true)
            end)
        end
...
function StarGate.Hook.PlayerInitialSpawn(p, reload)
    ...
    for name, data in pairs(StarGate.CFG.SYNC) do
        -- Prevents crashing (must be done everytime we send a umsg!)
        if (p and IsValid(p) and p:IsPlayer() or reload) then
            net.Start("StarGate_CFG")
...
hook.Add("KeyPress", "StarGate.KeyPress.SendGateData", function(p, key)
...
local function StarGate_OpenAll(ply)
    ...
    if (IsValid(ply)) then
        ply:PrintMessage(HUD_PRINTCONSOLE, "All irises are opened!")
    else
        print("All irises are opened!")
    end
end
...
local function CanPlayerSpawnSENT(player, EntityName)
    ...
    if (not sent.SpawnFunction) then return false end

    if (StarGate_Group and StarGate_Group.Error == true) then
        StarGate_Group.ShowError(player)
...
concommand.Add("cap_spawnsent", function(ply, cmd, args)
    CAP_Spawn_SENT(ply, args[1])
end)
...
concommand.Add("cap_spawnswep", function(ply, cmd, args)
    CAP_Spawn_Weapon(ply, args[1])
end)
...
local function InternalSpawnNPC(Player, Position, Normal, Class, Equipment)
    ...
    if (not NPCData) then return end --[[if ( IsValid( Player ) ) then
...
concommand.Add("cap_spawnnpc", function(ply, cmd, args)
    CAP_Spawn_NPC(ply, args[1], args[2])
end)
...
function StarGate.NotSpawnable(class, player, mode, nomsg)
    ...
    if (StarGate.CFG:Get(mode .. "_groups_only", class, false)) then
        ...
        if (table.HasValue(tbl, "exclude_mod")) then
            exclude = true
            disallow = false
        end
