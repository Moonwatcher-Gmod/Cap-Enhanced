...
ENT.PrintName = "Ore deposit: Naquadria"
ENT.Author = "Soren"
...
function ENT:Draw()
    ...
    if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
        hook.Add("HUDPaint", tostring(self.Entity) .. "ore", function()
            ...
            -- end
            draw.SimpleText("Naquadria", "center", ScrW() / 2 + 70 + w, ScrH() / 2 + 65 - h, Color(255, 255, 255, 255), 0)
            draw.SimpleText("Nq+", "DermaLarge", ScrW() / 2 + 40 + w, ScrH() / 2 + 85 - h, Color(42, 42, 42, 255), 0)
        end)
