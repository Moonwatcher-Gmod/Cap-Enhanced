...
ENT.PrintName = "Chair ZPM Hub"
ENT.Author = "Llapp, cooldudetb, Rafael De Jongh"
ENT.Category = "Stargate Carter Addon Pack"
ENT.WireDebugName = "Chair ZPM Hub"
ENT.Spawnable = false
...
if SERVER then
    ...
    function ENT:Initialize()
        self.Entity:SetModel("models/soren/zpm_slot/zpm_slot.mdl")
        ...
        self.rateOfChange = 0
        self.Overload = 0
        self.MaxTransferRate = 120000
        self.CurrentOverload = 1
        self.OverloadFactor = 0
        self.ZapTime = 0
        self.SoundPitch = 46
        self.Chair = nil
        self.ZPM_Percentage = 0
        self.ZPM_Active = 0
		------------------
    ...
    function ENT:Think()
        ...
        if (self.rateOfChange == 2800000 or self.rateOfChange == 5600000 or self.rateOfChange == 8400000 ) then
            self.rateOfChange = 0
        end
        ...
        self.SoundPitch = 46 + (self.Overload*0.6)
        ...
        ----------------Failsafe and overload-------------------
        if (StarGate.CFG:Get("hub_overload", "allow_overload", false)) then
            if (self.Active) then
                --self.CurrentOverload = (self.rateOfChange - self.MaxTransferRate)/100000
                ...
                if (self.rateOfChange < self.MaxTransferRate) then
                    self.Overload = math.Clamp(self.Overload - 0.05, 0, 100)
                end
            else
                    self.Overload = math.Clamp(self.Overload - 1, 0, 100)
            end
        else
            if (self.Overload > 0) then
                self.Overload = math.Clamp(self.Overload - 0.05, 0, 100)
            end
        end
        ---------------------------------------------
    ...
    function ENT:Output(perc, eng)
        ...
        self.ZPM_Percentage = perc
        self.ZPM_Active = add
        self.Entity:SetNWString("add", add)
        self.Entity:SetNWString("perc", perc)
        self.Entity:SetNWString("eng", self.rateOfChange*100)
    ...
    function ENT:ZPMsMovement()
        ...
        if (self.ZPM.IsValid and self.ZPM.Dist ~= self.ZPM.Dir) then
            ...
            if (self.ZPM.Dir < self.ZPM.Dist) then
                ...
                self.ZPM.Ent.IsConnectedToHub = true
            ...
            elseif (self.ZPM.Dir > self.ZPM.Dist) then
                ...
                self.ZPM.Ent.IsConnectedToHub = false
                self.ZPM.Dist = self.ZPM.Dist + spd
            ...
            self.ZPM.Ent:SetPos(pos + self.Entity:GetRight() * (self.Position.R) + (self.Entity:GetUp() * (20 * self.ZPM.Dist) + (self.Entity:GetUp()*7*(1-self.ZPM.Dist))) + self.Entity:GetForward() * (self.Position.F))
        ...
        if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
            hook.Add("HUDPaint", tostring(self.Entity) .. "SGCH", function()
                ...
                if (SGLanguage ~= nil and SGLanguage.GetMessage ~= nil) then
                    ...
                    draw.DrawText("Flowrate", "center2", ScrW() / 2 + 40 + w, ScrH() / 2 + 100 - h, Color(209, 238, 238, 255), 0)
            		draw.DrawText("Overload", "center2", ScrW() / 2 + 40 + w, ScrH() / 2 + 138 - h, Color(209, 238, 238, 255), 0)
                    draw.DrawText("Capacity", "center2", ScrW() / 2 + 40 + w, ScrH() / 2 + 178 - h, Color(209, 238, 238, 255), 0)
                end

                if (IsValid(self.Entity)) then
                    add = self.Entity:GetNWString("add")
                    perc = self.Entity:GetNWString("perc")
                    flowrate = self.Entity:GetNWString("eng")
                    overload = self.Entity:GetNWString("overload")
                end

                local realpower = "nil"

                if(flowrate >=1000000) then
                    realpower = tostring((string.format("%G",flowrate/1000000)).." MW")
                elseif (flowrate >=1000) then
                    realpower = tostring((string.format("%G",flowrate/1000)).." KW")
                else
                    realpower = tostring((string.format("%G",flowrate)).." W")
                end
                ...
                draw.SimpleText(tostring(realpower), "center", ScrW() / 2 + 40 + w, ScrH() / 2 + 118 - h, Color(255, 255, 255, 255), 0)
                draw.SimpleText(tostring(perc) .. "%", "center", ScrW() / 2 + 40 + w, ScrH() / 2 + 198 - h, Color(255, 255, 255, 255), 0)
                draw.SimpleText(tostring(overload) .. "%", "center", ScrW() / 2 + 40 + w, ScrH() / 2 + 158 - h, Color(255, 255, 255, 255), 0)
            end)
        end
