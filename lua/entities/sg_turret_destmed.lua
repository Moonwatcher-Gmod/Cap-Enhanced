...
if SERVER then
    ...
    ENT.BarrelPos = {Vector(0, 20, 20), Vector(0, 0, 32), Vector(0, -20, 20)}
    ...
    function ENT:Initialize()
        ...
        if (WireAddon) then
            self.Inputs = WireLib.CreateInputs(self.Entity, {"Fire [NORMAL]", "Active [NORMAL]", "Vector [VECTOR]", "Entity [ENTITY]", "Repair"})
        end
    ...
    function ENT:Shoot()
        ...
        if (energy > self.energy_drain or not self.HasResourceDistribution) then
            ...
            e:PrepareBullet(-1 * cann:GetForward(), 10, 16000, 15, {self.Cann1, self.Cann2, self.Cann3, self.Turn, self.Stand})
            ...
            timer.Simple(math.random(10, 16) / 10, function()
                self.CanFire = true
            end)
        end
    end
