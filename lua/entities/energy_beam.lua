...
if SERVER then
    ...
    function ENT:Think()
        ...
        if (self:GetLength() > 0 or (isSourceValid and isStartValid)) then
            self:Extend()
        end
        ...
        if (isSourceValid == false) then
            ...
            elseif (self.wasStartValid == false) then
                ...
                newBeam:Setup(self.Source.Entity, table.Copy(self.Start), self.Damage, self.speed, self.colour, self.effect)
                newBeam:Spawn()
                newBeam:Activate()
    ...
    function ENT:SendPosToClient()
        if (IsValid2(self.Laser) == false or IsValid2(self.End.Entity) == false) then return end
    ...
    function ENT:UpdateStartPos()
        if (self.Start:IsValid() == false) then return false end
    ...
    function ENT:GetLength()
        if (IsValid2(self.End.Entity) == false or IsValid2(self.Laser) == false) then
            return 0
        else
    ...
    function ENT:IsSourceValid()
        ...
        if (IsValid2(sourceEnt)) then
            ...
            if (sourceEnt.trace == nil and isSourceFiring == true) then
                return true
            elseif (sourceEnt.trace) then
                ...
                if (IsValid2(traceEnt)) then
                    ...
                    if (traceEnt:GetClass() == "event_horizon") then
                        ...
                        if (remoteEH == self.Start.Entity) then return true end
    ...
    function ENT:Extend()
        ...
        StarGate:EmitHeat(hitPos, (self.Damage.amount / 50) * self.Ftime, self.Damage.radius, owner)
        ...
        if (IsValid2(hitEnt)) then
            ...
            elseif (hitEnt:GetClass() == "event_horizon") then
                ...
                if (startEnt ~= remoteEH and remoteEH ~= nil) then
            ...
            if IsValid2(self.Entity) then
                util.BlastDamage(self.Entity, owner, hitPos, self.Damage.radius, self.Damage.amount * self.Ftime)
            end
        end
    end
    ...
    function ENT:OnHitEventHorizon(eventHorizon, hitPos)
        ...
        if (IsValid2(self.RemoteGateBeam) == false or self.RemoteGateBeam.Start.Entity ~= remoteEH) then
            self.RemoteGateBeam = ents.Create("energy_beam")
            self.RemoteGateBeam:Setup(self, Start, self.Damage, self.speed, self.colour)
        else
    ...
    function ENT:Shrink()
        if (IsValid2(self.Laser) == false) then return false end
