...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        -- Config
        self.Radius = 10 + 15 * self.Size --StarGate.CFG:Get("staff","radius",50);
        self.Damage = 10 + 100 * self.Size --StarGate.CFG:Get("staff","damage",150);
        self.MaxPasses = 3 --StarGate.CFG:Get("staff","maxpasses",5);
        self.Passes = 1
...
if CLIENT then
    ...
    function ENT:Think()
        ...
        if ((time - self.Created >= 0.1 or self.InstantEffect) and time - (self.Last or 0) > 0.3) then
            ...
            if (math.abs(dot_prod) < 0.5 and dot_prod ~= 0 and len < 300) then
