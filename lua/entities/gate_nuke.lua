...
if SERVER then
    ...
    -- THIS MUST BE CALLED BEFORE IT'S SPAWNED
    ...
    function ENT:Initialize()
        ...
        if self.Instancing then
            elf.Inphase = self.Entity:GetInstance()
        end
        -- Set Up the effect
        ...
        for k, v in pairs (player.GetAll()) do
            v:EmitSound("ambient/levels/streetwar/city_battle11.wav")
            v:EmitSound("ambient/levels/labs/teleport_preblast_suckin1.wav")
    ...
    ENT.GateList = {"stargate_sg1", "stargate_atlantis", "stargate_universe", "stargate_orlin", "stargate_movie", "stargate_tollan", "stargate_infinity"}

    ENT.DHDList = {"dhd_sg1", "dhd_atlantis", "dhd_universe", "dhd_infinity"}
...
if CLIENT then
    ...
    -- All Credits for this goes to Jinto :}) -- Moustache
    function ENT:NukeSunBeams()
        ...
        if dot > 0 then
            DrawSunbeams(0.95, 0.5 * dot, 0.075, screenpos.x / ScrW(), screenpos.y / ScrH())
        end

        return true
    end

    -- I could just do this with network varibles but there sent the same way soo... meh.
    local function NukeSunBeamInfo(Info)
...
util.PrecacheSound("ambient/levels/streetwar/city_battle11.wav")
util.PrecacheSound("ambient/levels/labs/teleport_preblast_suckin1.wav")
util.PrecacheSound("ambient/explosions/explode_6.wav")
util.PrecacheSound("ambient/explosions/exp1.wav")
    ...
    function ENT:Initialize()
        ...
        if (self.Inphase) then
            hook.Add("RenderScreenspaceEffects", "NukeSunBeams", function()
                self:NukeSunBeams()
            end)
