...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Wraith Plasma Cannon"
...
if SERVER then
    ...
    ENT.Sounds = {
        Shoot = Sound("weapons/wraith_hive_shoot.mp3")
    }
    ...
    function ENT:Initialize()
        ...
        self.Entity:SetName("Wraith Plasma Cannon")
    ...
    function ENT:SpawnFunction(ply, tr)
        ...
        local PropLimit = GetConVar("CAP_wraith_cannon_max"):GetInt()

        if (ply:GetCount("CAP_wraith_cannon") + 1 > PropLimit) then
            ply:SendLua("GAMEMODE:AddNotify(\"Wraith Plasma Cannon limit reached!\", NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )")

            return
        end

        ent = ents.Create("wraith_plasma_cannon")
        ...
        ply:AddCount("CAP_wraith_cannon", ent)
    ...
    function ENT:Shoot()
        ...
        if (energy > self.energy_drain or not self.HasRD) then
            ...
            local e = ents.Create("wraith_pulse")
	...
	if(Ent.EntityMods and Ent.EntityMods.AsgardDupeInfo.WireData) then
        ...
        if (IsValid(ply)) then
            ply:AddCount("CAP_wraith_cannon", self.Entity)
        end
    ...
    if (StarGate and StarGate.CAP_GmodDuplicator) then
        duplicator.RegisterEntityClass("wraith_plasma_cannon", StarGate.CAP_GmodDuplicator, "Data")
    end
end
