...
surface.CreateFont("Marlett_Open", {
    font = "Marlett",
    size = 46,
    weight = 800,
    antialias = true,
...
surface.CreateFont("Marlett_Err", {
    font = "Marlett",
    size = 19,
    weight = 800,
    antialias = true,
...
surface.CreateFont("Marlett_Error", {
    font = "Marlett",
    size = 56,
    weight = 800,
    antialias = true,
...
ENT.CusModelRS = {
    ["models/blacknecro/tv_plasma_4_3.mdl"] = {0.055}, 
    ["models/cheeze/pcb/pcb5.mdl"] = {0.039, -0.125},
    ["models/cheeze/pcb/pcb6.mdl"] = {0.0585, -0.175},
    ["models/cheeze/pcb/pcb8.mdl"] = {0.078, -0.225},
    ["models/props/cs_assault/billboard.mdl"] = {0.14, -0.35},
    ["models/props/cs_militia/reload_bullet_tray.mdl"] = {0.00605},
    ["models/props/cs_office/computer_monitor.mdl"] = {0.0205},
    ["models/props/cs_office/tv_plasma.mdl"] = {0.04, -0.075},
    ["models/props_lab/monitor01b.mdl"] = {0.0091, 0, -0.125}
}
...
function ENT:Initialize()
    ...
    if (monitor and model ~= "models/props_lab/monitor01a.mdl") then
        ...
        self:ScreenInit(512, 384, monitor.offset + monitor.rot:Forward() * (-512 / 2 * RS*2) + monitor.rot:Right() * (-384 / 2 * RS*2) + monitor.rot:Right() * PFixX + monitor.rot:Up() * PFixZ, monitor.rot, RS*2)
    else
...
function ENT:Screen()
    ...
    if not self:GetNW2Bool("ServerConnected", false) or not IsValid(self.Server) then
        ...
        if self.NoSignalX + 210 > 512 then
            self.NoSignalXDir = -1
        elseif self.NoSignalX < 0 then
            self.NoSignalXDir = 1
        end

        self.NoSignalY = self.NoSignalY + 30 * FrameTime() * self.NoSignalYDir

        if self.NoSignalY + 130 > 384 then
            self.NoSignalYDir = -1
        elseif self.NoSignalY < 0 then
            self.NoSignalYDir = 1
        end
...
function ENT:Think()
    ...
    if IsValid(self.Server) then
        ...
        if self.Server:GetNW2Bool("SelfDestruct", false) and timer ~= self.DestructTime then
            if self.DSound then
                self:EmitSound("glebqip/self_destruct_beep3.wav", 65, 100, 0.2)
            end
