...
properties.Add("SGCScreen.CodeMenu", {
    MenuLabel = "Codes manager",
    Order = -10,
    MenuIcon = "icon16/page_key.png",
    Filter = function(self, ent, ply)
        if (not IsValid(ent) or not IsValid(ply) or ent:GetClass() ~= "sgc_server" or ent:GetNWBool("GateSpawnerProtected", false) or ent:GetNWBool("DisMenu", false) or ply ~= ent:GetNWEntity("Owner")) then return false end
        if (not gamemode.Call("CanProperty", ply, "sgcscreenmodify", ent)) then return false end
...
hook.Remove("StarGate.Teleport", "SGC_Computer_v1",function(ent,gate,test,blk)
    ...
    function ENT:FindFineGates(gate)
        ...
        for _, v in pairs(ents.FindByClass("stargate_*")) do
            ...
            if supergate then
                --it's good supergate!
                if #address ~= 0 and v ~= gate and not v.IsGroupStargate and not v.GatePrivat then
    ...
    function ENT:FindNearestGate(pos, dist)
        ...
        for k, v in pairs(ents) do
            if (v.IsGroupStargate and IsValid(v.Ring)) then
                local e_pos = v:GetPos()
                local dist = (e_pos - pos):Length()
    ...
    function ENT:Think()
        ...
        if self.On and self.State ~= -1 then
            ...
            if time > 2 and self.State == 0 then
                self.State = -2
            end
        ...
        if (IsValid(gate)) and self.On and self.State == -1 then
            if CurTime() - self.AddressCheck > 10 then
                ...
                --First iteration for find bounding box of all gates
                for _, gate in ipairs(self.Gates) do
                    local pos = gate:GetPos()

                    if not xmin or xmin > pos.x then
                        xmin = pos.x
                    end

                    if not xmax or xmax < pos.x then
                        xmax = pos.x
                    end

                    if not ymin or ymin > pos.y then
                        ymin = pos.y
                    end

                    if not ymax or ymax < pos.y then
                        ymax = pos.y
                    end
                end
                ...
                if #addr < 9 and not addr:find("#") then
                    addr = addr .. "#"
                end
            ...
            if not self.RequireTwoPlayers or not self.SelfDestruct and table.Count(self.SelfDestructCodes) == 1 or self.SelfDestruct and table.Count(self.SelfDestructResetCodes) == 1 then
                need = 1
            end
            ...
            elseif enter == need and self.SelfDestruct then
                if IsValid(self.Bomb) then
                    self.Bomb:AbortDetonation(self.Bomb.abortCode)
                end
            ...
            elseif (self.DialErr or self.LockErr) then
                -- we fail dial
                if not open and not inbound and chevron <= 0 and #self.DialingAddress >= 6 then
            ...
            --Dial error check
            --Reset err symbol if we don't need it
            if chevron == 0 and self.ErrorSymb and self.DCError == 0 then
                self.ErrorSymb = nil
            end
            ...
            if movie and self.SymbolAnim2 and not LastSecond then
                dadddelta = dadddelta .. dialdsymb
            end
            ...
            --GDO scripts
            --[[ and self.IDCReceiver.LockedGate ~= self.IDCReceiver.Entity]]
            if inbound and IsValid(self.IDCReceiver) then
    ...
    function ENT:Touch(ent)
        if not IsValid(self.LockedGate) and (ent.IsGroupStargate) and not IsValid(ent.SGCServer) and IsValid(ent.Ring) then
            self.LockedGate = ent
            self.LockedGate:TriggerInput("SGC Type", 1)
            ent.SGCServer = self
    ...
    function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
        if (StarGate.NotSpawnable("sgc_screen", ply, "tool")) then
            Ent:Remove()
        ...
        if (IsValid(ply)) then
            if (ply:GetCount("CAP_sgc_servers") >= GetConVar("sbox_maxsgc_server"):GetInt()) then
                ply:SendLua("GAMEMODE:AddNotify(")
                Hit"someString"
                the"someString"
                SGC", NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )"
            else
                Ent:Remove()
...
print("--END CHECK--")
    ...
    concommand.Add("sgc_screen_menu", function()
        SGC_Codes_Mgr()
    end)
