...
hook.Add("HUDPaint", "Ring.Hook.HUDPaint.ShowAddressAndNames", function()
    local x, y = gui.MousePos()

    -- Avoids this popping up, if the dial dialogue is opened
    if (x == 0 and y == 0) then
        local p = LocalPlayer()
...
function PANEL:Init()
    ...
    self.L1:SetText("Name this ring (numbers only!):")
...
hook.Add("HUDPaint", "RingTransporterHUDPaint", function()
...
usermessage.Hook("RingTransporterTele", function(data)
    go = data:ReadBool()
    start = CurTime()

    if (go) then
        timer.Create("RingTeleSafeTimer", 5, 1, function()
            start = nil
        end)
    end
end)