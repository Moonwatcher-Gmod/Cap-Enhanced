...
ENT.Type = "anim"
ENT.Base = "base_anim"
ENT.PrintName = "Ion Generator"
ENT.Author = "Soren"
ENT.Contact = ""
ENT.Category = "Moonwatcher: Machines"
ENT.WireDebugName = "Ion Generator"
ENT.Spawnable = false
...
if SERVER then
    ...
    function ENT:Initialize()
        self.Entity:SetModel("models/starwars/syphadias/props/sw_tor/bioware_ea/props/nar_shadda/nar_holo_bench.mdl")
        self.Entity:PhysicsInit(SOLID_VPHYSICS)
        self.Entity:SetMoveType(MOVETYPE_VPHYSICS)
        self.Entity:SetSolid(SOLID_VPHYSICS)
        self.MaxEnergy = 40000
        self.Energy = 40000
        self:AddResource("energy", StarGate.CFG:Get("naq_gen_mk1", "energy", 10000))
        self.Generate = 800
        self.GenMulti = 30
        self:CreateWireInputs("ON/OFF", "Disable Use","Heat","Gate [ENTITY]")
        self:CreateWireOutputs("Active", "Naquadah", "Naquadah %", "Energy", "Health")
        self.EntHealth = 100
    ...
    function ENT:SpawnFunction(pl, tr)
        ...
        local e = ents.Create("ion_generator")
    ...
    function ENT:Think()
        ...
        if (self:GetWire("Heat") > 0) then
            local gate = self:GetWire("Gate")
            if (IsValid(gate)) then

                if (gate.excessPowerLimit == nil) then
                gate.excessPowerLimit = 50000*10
                end
                ...
                gate.excessPower = gate.excessPower + 50000
    ...
    if (StarGate and StarGate.CAP_GmodDuplicator) then
        duplicator.RegisterEntityClass("ion_generator", StarGate.CAP_GmodDuplicator, "Data")
    end
end
    ...
    function ENT:Draw()
        ...
        if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
            hook.Add("HUDPaint", tostring(self.Entity) .. "ION", function()
                ...
                draw.DrawText("ION GEN", "header", ScrW() / 2 + 54 + w, ScrH() / 2 + 41 - h, Color(255, 255, 255, 255), 0)
                draw.DrawText("Status", "center2", ScrW() / 2 + 40 + w, ScrH() / 2 + 65 - h, Color(209, 238, 238, 255), 0)
                draw.DrawText("Ion Cells", "center2", ScrW() / 2 + 40 + w, ScrH() / 2 + 115 - h, Color(209, 238, 238, 255), 0)
