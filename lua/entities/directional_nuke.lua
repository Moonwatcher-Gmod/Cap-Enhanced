...
if SERVER then
    ...
    function ENT:Use(ply)
        if (self.Owner == ply) then
            ...
            if timer.Exists("Count" .. self:EntIndex()) then
                timer.Destroy("Count" .. self:EntIndex())
            end

            if timer.Exists("Tick" .. self:EntIndex()) then
                timer.Destroy("Tick" .. self:EntIndex())
            end
...
if CLIENT then
    ...
    function VGUI:Init()
        ...
        DermaPanel:SetTitle("Shaped Charge Timer")
        ...
        timlab:SetPos(20, 40)
        timlab:SetText("Seconds to explosion:")
