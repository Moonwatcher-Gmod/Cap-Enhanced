    ...
    function ENT:StartTouch(ent)
        if ent:IsPlayer() then
            ...
            if (self.Touching == 1) then
                self:SetNWBool("Display", true)
                self:SetWire("Activated", true)
            end

            if timer.Exists(self:EntIndex() .. "NotTouch") then
                timer.Destroy(self:EntIndex() .. "NotTouch")
            end
        end
    end
...
if CLIENT then
    ...
    ENT.Stars = StarGate.MaterialFromVMT("Stars", [["Sprite"
	{
    ...
    function ENT:Initialize()
        ...
        for i = 1, 200 do
            ...
            if (math.random(0, 1) == 0) then
                col = Vector(math.random(150, 200), math.random(0, 30), math.random(0, 30))
            else
                col = Vector(math.random(0, 30), math.random(0, 30), math.random(150, 200))
            end
