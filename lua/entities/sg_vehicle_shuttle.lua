...
if SERVER then
    ...
    --################# Get out the jumper@RononDex
    function ENT:ExitShut()
...
if CLIENT then
    ...
    --############# Recieve data from init@RononDex
    ...
    function PrintHUD()
        ...
        if (IsValid(self)) then
            if ((IsValid(shuttle)) and (shuttle == self)) then
                local healthpercent = math.Round(self:GetNWInt("health") / self:GetNWInt("maxhealth") * 100) -- test fix
                draw.WordBox(8, hudpos.healthw, hudpos.healthh, "Hull: " .. healthpercent .. "%", "ScoreboardText", Color(50, 50, 75, 100), Color(255, 255, 255, 255))
