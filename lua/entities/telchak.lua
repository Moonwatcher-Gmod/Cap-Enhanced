...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        -- yay do it more often than usual think
        timer.Create(self.Entity:EntIndex() .. "LowHelath", 0.05, 0, function()
            for _, v in pairs(self.SickPlayers) do
                if (IsValid(v) and v:IsPlayer()) then
                    local health = v:Health()

                    if (health > math.Rand(20, 60)) then
                        v:SetHealth(health - 2)
                    else
                        local new_t = {}
    ...
    function ENT:OnRemove()
        ...
        if timer.Exists(self.Entity:EntIndex() .. "LowHelath") then
            timer.Destroy(self.Entity:EntIndex() .. "LowHelath")
        end
    end
                    ...
                    if v:IsPlayer() and not v:HasGodMode() then
                        if (len < 300) then
                            ...
                            if (health > math.Rand(190, 200)) then
                                if not table.HasValue(self.SickPlayers, v) then
                                    table.insert(self.SickPlayers, v)
                                end
                            else
                                v:SetMaxHealth(200)
