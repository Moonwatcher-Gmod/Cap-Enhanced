...
function ENT:Initialize()
    ...
    self:CreateWireInputs("Target [VECTOR]")
...
--########## F302's aren't invincible are they? @RononDex
    ...
    if (self.NextUse.Cockpit < CurTime()) then
        if (IsValid(self.CockpitAnim)) then
            if (not (self.CockpitOpen)) then
                ...
                if (self.Cockpit.CollisionGroup) then
...
function ENT:Think()
    ...
    self:SetWire("Health", self.EntHealth)
    self:SetNWInt("clhealth", self.EntHealth)
...
--########### Missile firing stuff @RononDex
function ENT:FireMissile(pos)
...
-- Dummy function for missiles
function ENT:ShowOutput()
end

--######### Wire Inputs @ RononDex
...
--############### Spawn Missile Props @RononDex
function ENT:SpawnMissile()
...
--####### Spawn the turret entities @RononDex
function ENT:Turrets(p)
...
function ENT:SpawnSeats(p)
    ...
    if CPPI and IsValid(p) and pilotseat.CPPISetOwner then
        pilotseat:CPPISetOwner(p)
    end
    ...
    if CPPI and IsValid(p) and passseat.CPPISetOwner then
        passseat:CPPISetOwner(p)
    end
