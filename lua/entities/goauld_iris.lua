...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        -- pewpew support
        if (pewpew and pewpew.NeverEverList and not table.HasValue(pewpew.NeverEverList, self.Entity:GetClass())) then
            table.insert(pewpew.NeverEverList, self.Entity:GetClass())
        end
    end
    ...
    function ENT:Think()
        ...
        if (self.IsActivated) then
            if (self.HasRD) then
                ...
                if IsValid(gate) and gate.IsStargate and not gate:HaveEnergy(true, true) then
                    self:TrueActivate(true)
                end
            end
        end
    ...
    function ENT:Close()
        ...
        timer.Create(id, 1.5, 1, function()
            if (IsValid(e)) then
                snd:PlayEx(90, math.random(88, 93))
            end
        end)
        ...
        if (self.NextAction <= CurTime()) then
            ...
            if (self.IsActivated or deactivate) then
                ...
                if (self.HasRD) then
                    ...
                    if IsValid(gate) and gate.IsStargate and not gate:HaveEnergy(true, true) then
                        if (self.Sounds and self.Sounds.Fail) then
                            self.Entity:EmitSound(self.Sounds.Fail, 90, math.random(90, 110))
                        end
    ...
    function ENT:IsBusy()
        if (self.NextAction <= CurTime()) then return false end
    ...
    function ENT:TriggerInput(k, v)
        if (k == "Activate" and ((not self.IsActivated and v >= 1) or (self.IsActivated and v == 0))) then
            self:Toggle()
