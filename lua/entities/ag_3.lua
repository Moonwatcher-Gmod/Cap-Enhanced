...
if SERVER then
    ...
    function ENT:OnRemove()
        ...
        if IsValid(self.InfoTar) then
            self.InfoTar:Remove()
        end

        if IsValid(self.PlasmaBeam) then
            self.PlasmaBeam:Remove()
        end
    end
    ...
    function ENT:Think(ply)
        ...
        if (self.Canfire) then
            ...
            if self.Firing then
                self.InfoTar:SetPos(self.AimPoint)
            end
    ...
    function ENT:CreateSmallLasers()
        if (not self.InfoTar and not self.PlasmaBeam) then
            ...
            if (not (data and data.Pos)) then
                data.Pos = self.Entity:GetPos() + self.Entity:GetForward() * 20
            end
