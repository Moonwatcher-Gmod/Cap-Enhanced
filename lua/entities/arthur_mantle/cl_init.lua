...
hook.Add("PrePlayerDraw", "StarGate.ArthurCloak", function(p)
    ...
    if (cloaked ~= NULL or sodan_cloaked ~= NULL) then
        ...
        elseif (not sodan_self and not cloaked_self) then
            if (cloaked or sodan_cloaked) then
                cloak = true
            end
        end
...
local function checkfont(self)
    ...
    if (self.AFont != self.TFont) then
        validfont = false
    end

    if (self.SFont != self.GFont) then
        validfont = false
    end

    if (validfont == nil) then
        validfont = false
    end
