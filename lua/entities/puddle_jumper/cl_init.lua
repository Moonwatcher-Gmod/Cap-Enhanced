...
function ENT:CreateHUD()
    if (not (self.HUD and self.HUD.Activate and self.HUD.Deactivate)) then
        -- Delete invalid but existant previous HUD
        if (self.HUD) then
            self.HUD:Remove()
        end
...
--############# Recieve Data from the Server @RononDex
...
--#########################  Overly complex think function @ RononDex,LightDemon,aVoN
function ENT:Think()
    local p = LocalPlayer()
