...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        self:CreateWireInputs("Origin [VECTOR]", "Dest [VECTOR]", "Teleport Everything", "Send", "Retrieve", "Radius", "Disable Use")
    ...
    function ENT:FindObjects(pos, quick)
        ...
        for _, e in pairs(ents.FindInSphere(pos, self.Radius)) do
            if (e:IsValid()) then
                -- Only players?
                ...
                elseif (self.TeleportEverything and not self.Exceptions[e]) then
                    ...
                    if (self:ValidForTeleport(e)) then
                        ...
                        if (not quick) then
                            ...
                            -- So much to teleport? NAH! (Maybe later with the stargate teleport stuff)
                            if (#entities < 50) then
                                for k, v in pairs(entities) do
                                    if (self.Exceptions[v] or v == self.Entity) then
                                        allow = false
                                        break
                                    end
                                ...
                                if (allow) then
                                    for _, v in pairs(entities) do
                                        if (self:ValidForTeleport(v)) then
                                            objects[v] = v
                                        end
                                    end
                                end
    ...
    function ENT:TriggerInput(k, v)
        ...
        elseif(k == "Radius") then
            self.Radius = math.Clamp(v,1,130)
    ...
    function ENT:Teleport(from, to, ply, gps_ent)
        ...
        -- first check for jammming devices, we can telepor from and to only if jamming is offline (madman07)
        local radius = 1024 -- max range of jamming, we will adjust it later
        local jamming_online = false
        ...
        for _, v in pairs(ents.FindInSphere(from, radius)) do
            if IsValid(v) and v.CapJammingDevice then
                if v.IsEnabled then
                    ...
                    -- ow jamming, we cant do anything
                    if (dist < v.Size) then
                        -- but we are the owner so we know codes :D it does nothign, so create some value...
                        if v.Immunity and v.Owner == self.Owner then
                            local a
                        else
                            jamming_online = true
                        end
        ...
        if not jamming_online then
            ...
            if (IsValid(gps_ent)) then
                self.Exceptions[gps_ent] = true
            end
            ...
            sound.Play(self.O_sound, from, 100, 100)

            local diff = to - from

            for _, v in pairs(entities) do
                local start = v:GetPos()
                local dest = diff + start -- Offset so you never get stuck in ground
                dest.z = dest.z + 1
...
//                     DoTeleportTimer(v, dest, color.r, color.g, color.b, color.a, rendmode)
						    
//                     timer.Simple( 1, function() 
//                     	v:SetRenderMode(rendmode)
//                     	v:SetColor(Color(color.r, color.g, color.b, color.a))
...
//                     sound.Play(self.D_sound, to, 100, 100)
//                     fx:SetOrigin(dest)
//                     fx:SetScale(0) -- "Spit out" - a.k.a. "beam me down". Don't spit bitch
//                     -- Note to myself: Why am I so damn sarcastic and vogue?
//                     util.Effect("teleport_effect", fx, true, true)
        ...
        elseif (type == 0) then
            ...
            sendButton.DoClick = function(btn)
                ...
                if (target and dest) then
                    ...
                    GAMEMODE:AddNotify(SGLanguage.GetMessage("asgardtp_noact"), NOTIFY_ERROR, 5)
