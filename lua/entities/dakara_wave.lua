...
function ENT:Setup(centre, immuneEnts, target, propagate, maxradius, originGate)
   self.Target = target;
   self:SetPos(centre)
...
function ENT:PropagateThroughGate(gate)
	for _,v in pairs(ents.FindByClass("stargate_*")) do
		if(v.IsStargate and v!=gate and v:GetClass() != "stargate_supergate" and v:GetClass() != "stargate_orlin" and v:GetClass() != "stargate_universe" and not v.GateGalaxy) then
			if (self.OriginGate and v == self.OriginGate) then continue end
            local wave = ents.Create("dakara_wave")
			wave:Setup(v:GetPos(), self.immuneEnts, self.Target, false, self.MaxRadius, self.OriginGate)
			wave:Spawn()
			wave:Activate()
		end
	end
