...
function ENT:GetSubBeamPos(beam)
    ...
    local attachmentIDs = {self.Entity:LookupAttachment("emitter1"), self.Entity:LookupAttachment("emitter2"), self.Entity:LookupAttachment("emitter3"), self.Entity:LookupAttachment("emitter4"), self.Entity:LookupAttachment("emitter5"), self.Entity:LookupAttachment("emitter6")}
...
if SERVER then
    ...
    function ENT:UpdateWireOutputs()
        ...
        if (self.isFiring and self.remoteGate.excessPowerLimit and self.remoteGate.excessPower) then
            local energyRequired = self.remoteGate.excessPowerLimit - self.remoteGate.excessPower
    ...
    function ENT:Startup()
        ...
        if (self.HasRD) then end -- Allow the overloader to store the energy it needs for one second of fire --self:AddResource("energy", self.energyPerSecond)
    ...
    function ENT:Shutdown()
        ...
        self:StopFiring()
        if (self.HasRD) then end -- Remove energy storage while the device is not active --self:AddResource("energy", 1)
    ...
    function ENT:CreateSubBeams()
        if (self.emitterMarker == nil or self.emitterMarker:IsValid() == false) then
    ...
    function ENT:DestroySubBeams(subBeams)
        ...
        if (self.emitterMarker and self.emitterMarker:IsValid()) then
...
function ENT:FindAsuran(gat)
    ...
    function ENT:OnTakeDamage(damageInfo)
        if (damageInfo:GetInflictor():GetClass() == "env_laser" and IsValid(self.beam)) then
        ...
        if (self.isDestructing == false and self.isCombatSystemEnabled ~= true) then
    ...
    function ENT:Destruct()
        if (self.isDestructing) then return end
