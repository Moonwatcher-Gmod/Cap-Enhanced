...
ENT.PrintName = "Ore deposit: Trinium"
ENT.Author = "Soren"
...
if SERVER then
    ...
    function ENT:SpawnFunction(pl, tr)
        ...
        local e = ents.Create("oredeposit_trin")
...
function ENT:Draw()
    ...
    if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
        hook.Add("HUDPaint", tostring(self.Entity) .. "ore", function()
            ...
            -- end
            draw.SimpleText("Trinium", "center", ScrW() / 2 + 70 + w, ScrH() / 2 + 65 - h, Color(255, 255, 255, 255), 0)
            draw.SimpleText("Tr", "DermaLarge", ScrW() / 2 + 40 + w, ScrH() / 2 + 85 - h, Color(205, 255, 255, 255), 0)
        end)
