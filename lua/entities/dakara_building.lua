...
if SERVER then
    ...
    function ENT:OnRemove(ply)
        ...
        if IsValid(self.SecretDoor) then
            self.SecretDoor:Remove()
        end

        if IsValid(self.MainButton1) then
            self.MainButton1:Remove()
        end

        if IsValid(self.MainButton2) then
            self.MainButton2:Remove()
        end

        if IsValid(self.SecretButton1) then
            self.SecretButton1:Remove()
        end

        if IsValid(self.SecretButton2) then
            self.SecretButton2:Remove()
        end
    end
    ...
    function ENT:Anims(e, anim, delay, nosound, sound)
        if (e.CanDoAnim) then
            ...
            e:SetPlaybackRate(0.5)
            e:ResetSequence(e:LookupSequence(anim)) -- play the sequence

            --How long until we can do the anim again?
            timer.Create(anim .. e:EntIndex(), delay, 1, function()
                e.CanDoAnim = true
            end)
        end
    end
    ...
    function ENT:StartTouch(ent)
        if IsValid(ent) then
            if not table.HasValue(self.ProtectedByTouch, ent) then
                table.insert(self.ProtectedByTouch, ent)
            end
        end
    end
    ...
    function ENT:PrepareWeapon(power, d_ply, d_prp, d_veh, d_rep, d_npc, d_entity, d_range)
        ...
        if (d_prp == 1) then
            table.insert(self.Targets, "prop")
        end
        ...
        if (d_rep == 1) then
            table.insert(self.Targets, "replicator")
        end
        ...
        timer.Create("CreateWave" .. self:EntIndex(), 8, 1, function()
            if IsValid(self) then
                self.Entity:CreateWave()
            end
        end)

        timer.Create("CreateEffect" .. self:EntIndex(), 1, 1, function()
            if IsValid(self) then
                self.Entity:SpawnChargingEffect()
            end
        end)
    end
