    ...
    function ENT:Draw()
        ...
        if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
            hook.Add("HUDPaint", tostring(self.Entity) .. "StonDev", function()
                ...
                if IsValid(self.Entity) then
                    act = self.Entity:GetNWInt("Active", 0)
                end
...
if SERVER then
    ...
    function ENT:Disconnect(ent)
        ...
        if ent.Connected then
            ...
            if IsValid(pair) then
                ...
                --- disconnect second stone
                if IsValid(pair.Tablet) then
    ...
    -----------------------------------DEATH----------------------------------
    -- What happen if one of connected player will die?
    function PlayerDeath_Stone(ply)
    ...
    function ENT:FindDevice()
        ...
        for _, v in pairs(ents.FindByClass("comstones_tablet")) do
            if (v ~= self.Entity and v.Channel == self.Channel) then
                ston = v:FindStone()
                if IsValid(ston) then return v, ston end
            end
        end
    ...
    function ENT:SwapPlayers(pl1, pl2)
        ...
        for k, v in pairs(pl1:GetWeapons()) do
            table.insert(w1, v:GetClass())
        end

        local w2 = {}

        for k, v in pairs(pl2:GetWeapons()) do
            table.insert(w2, v:GetClass())
        end
        ...
        for k, v in pairs(w2) do
            if v == "infinitygauntlet" or v == "infinitygauntlet_empty" then
                --v:Remove()
            else
                pl1:Give(tostring(v))
            end
        ...
        if (IsValid(vh1)) then
            pl2:EnterVehicle(vh1)
        end

        if (IsValid(vh2)) then
            pl1:EnterVehicle(vh2)
        end
    end
