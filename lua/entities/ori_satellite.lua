...
if SERVER then
    ...
    function ENT:Initialize()
        ...
        self.APC = nil
        self.APCply = nil
        self.EntHealth = StarGate.CFG:Get("ori_satellite", "health", 500)
        ...
        timer.Create(self.Entity:EntIndex() .. "Anim", 33, 0, function()
        ...
        Wire_TriggerOutput(self.Entity, "Health", self.MaxHealth)
    ...
    function ENT:OnRemove()
        ...
        if timer.Exists("CantDamage" .. self.Entity:EntIndex()) then
            timer.Destroy("CantDamage" .. self.Entity:EntIndex())
        end
    ...
    function ENT:TriggerInput(variable, value)
        ...
        elseif (variable == "Shield") then
            self.WireShield = value
    ...
    function ENT:LowProrityThink()
        ...
        if IsValid(self.APC) then
            ...
            if IsValid(self.APCply) then
                if (self.APCply:KeyDown(IN_ATTACK2)) then
                    if (not self.Shield and self.StrengthShield == 100 and self.Pressed == false) then
                        self.Shield = self.Entity:SpawnShield()
                        self.Pressed = true
                    elseif (IsValid(self.Shield) and self.Pressed == false) then
        ...
        if (self.StrengthShield == 0 and self.Shield and self.Shield:IsValid()) then
            self.Entity:RemoveShield()
        end
        ...
        if (CurTime() - self.Time > 1) then
            ...
            if (self.Shield and self.Shield:IsValid()) then
                self.StrengthShield = self.StrengthShield - 100 / ShieldTime
            else
                self.StrengthShield = self.StrengthShield + 100 / ShieldTime
            end
    ...
    function ENT:Think(ply)
        ...
        if self.LowThink then
            self.Entity:LowProrityThink()
        end
    ...
    function ENT:Launch()
        ...
        timer.Create("CantDamage" .. self.Entity:EntIndex(), 5, 1, function()
            self.CantDamage = false
        end)
    end
