--[[
	industrial molecular construction device for GarrysMod10
...
function ENT:Draw()
    ...
    if (LocalPlayer():GetEyeTrace().Entity == self.Entity and EyePos():Distance(self.Entity:GetPos()) < 1024) then
        hook.Add("HUDPaint", tostring(self.Entity) .. "MCD", function()
            ...
            draw.DrawText("IMCD", "header", ScrW() / 2 + 70 + w, ScrH() / 2 + 43 - h - 74, Color(0, 255, 255, 255), 0)
...
function ENT:Think()
    ...
    if (self.Entity:GetNWBool("IdleSound")) then
        self.IdleSound:ChangePitch(100, 80, 50)
        self.IdleSound:SetSoundLevel(60)
        self.IdleSound:PlayEx(1, 77)
    else
        self.IdleSound:Stop()
    end
...
function VGUI:Init()
    ...
    ComboBox.Setup = function(self, classes)
        if (classes["cloaking_generator"]) then
            ComboBox:AddLine("Cloaking", "cloaking_generator"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "cloaking_generator"))
        end

        if (classes["shield_generator"]) then
            ComboBox:AddLine("Shield", "shield_generator"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "shield_generator"))
        end

        if (classes["jamming_device"]) then
            ComboBox:AddLine("Jamming Device", "jamming_device"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "jamming_device"))
        end

        if (classes["telchak"]) then
            ComboBox:AddLine("Telchak healing device", "telchak"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "telchak"))
        end

        if (classes["zpm_mk3"]) then
            ComboBox:AddLine("ZPM MK III", "zpm_mk3"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "zpm_mk3"))
        end

        if (classes["tollan_disabler"]) then
            ComboBox:AddLine("Tollan Weapon Disabler", "tollan_disabler"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "tollan_disabler"))
        end
        ...
        if (classes["arthur_mantle"]) then
            ComboBox:AddLine("Arthurs Mantle", "arthur_mantle"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "arthur_mantle"))
        end
        ...
        if (classes["fnp90"]) then
            ComboBox:AddLine("SG FN P90", "fnp90"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "fnp90"))
        end

        if (classes["weapon_asura"]) then
            ComboBox:AddLine("Asuran Gun", "weapon_asura"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "weapon_asura"))
        end

        if (classes["weapon_zat"]) then
            ComboBox:AddLine("Zat'nik'tel", "weapon_zat"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "weapon_zat"))
        end

        if (classes["sg_medkit"]) then
            ComboBox:AddLine("Atlantis Medical Kit", "sg_medkit"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "sg_medkit"))
        end

        if (classes["ancient_obelisk"]) then
            ComboBox:AddLine(SGLanguage.GetMessage("ancient_obelisk"), "ancient_obelisk"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "ancient_obelisk"))
        end

        if (classes["naquadah_bottle"]) then
            ComboBox:AddLine("Naquadah Bottle", "naquadah_bottle"):SetTooltip(SGLanguage.GetMessage("mcd_device_desc") .. "\n" .. SGLanguage.GetMessage("mcd_classname", "naquadah_bottle"))
        end
    ...
    MenuButtonClose.DoClick = function(btn)
        surface.PlaySound("sg/scanner/deactivate2.wav")
        DermaPanel:Remove()
    end
    ...
    surface.PlaySound("sg/scanner/deactivate2.wav")
        if (ComboBox:GetSelected()[1] and ComboBox:GetSelected()) then
            ...
            if (CheckBoxThing6:GetChecked()) then
                antiNC = 1
            end
            ...
            if (class == "tollan_disabler" or class == "cloaking_generator" or class == "shield_generator" or class == "jamming_device") then
                ...
                net.WriteBit(tobool(immunity))
                net.WriteBit(tobool(phaseshifting))
                net.WriteInt(NumSliderThingy2:GetValue(), 16)
                net.WriteBit(tobool(drawbubble))
                net.WriteBit(tobool(passing))
                net.WriteBit(tobool(containment))
                net.WriteBit(tobool(antiNC))
...
local VGUI_AbortScreen = {}

function VGUI_AbortScreen:Init()
    local DermaPanel = vgui.Create("DFrame")
    DermaPanel:SetSize(350, 100)
    DermaPanel:Center()
    DermaPanel:SetTitle("IMD options")
    DermaPanel:SetVisible(true)
    DermaPanel:SetDraggable(false)
    DermaPanel:ShowCloseButton(true)
    DermaPanel:MakePopup()
    ...
    MenuButtonClose:SetPos(190, 40)
    MenuButtonClose:SetSize(130, 45)
    ...
    local MenuButtonAbort = vgui.Create("DButton")
    MenuButtonAbort:SetParent(DermaPanel)
    MenuButtonAbort:SetText("Abort Construction")
    MenuButtonAbort:SetPos(30,40)
    MenuButtonAbort:SetSize(130, 45)

    MenuButtonAbort.DoClick = function(btn)
        surface.PlaySound("sg/scanner/deactivate2.wav")
        DermaPanel:Remove()
...
function VGUI_AbortScreen:SetEntity(e)
    self.Entity = e
    if (not IsValid(e)) then return end
end

vgui.Register("MCDAbort", VGUI_AbortScreen)
vgui.Register("MCDEntry", VGUI)

function VGUI_AbortScreen:SetPlayer(ply)
    self.ply = ply
    if (not IsValid(ply)) then return end
end


net.Receive("MCD_Abort", function(len)
    local e = net.ReadEntity()
    if (not IsValid(e)) then return end
    local Window = vgui.Create("MCDAbort")
    Window:SetMouseInputEnabled(true)
    Window:SetVisible(true)
    Window:SetEntity(e)
    --Window:SetPlayer(ply)
end)
