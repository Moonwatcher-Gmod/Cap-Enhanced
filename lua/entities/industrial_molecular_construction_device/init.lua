...
local MCDEntities = {
	...
	["ancient_obelisk"] = {
		["model"] = "models/zsdaniel/ancient-obelisk/obelisk.mdl",
		["check"] = "ancient_obelisk",
		["pos"] = 58.2,
	},
	["shield_generator"] = {
		["model"] = shield_model,
...
function ENT:Initialize()
	self.Entity:SetModel("models/soren/mcd2/mcd2.mdl");
	...
	self.Mul = 0;
	self.Start = false;
	self.RequireEnergy = StarGate.CFG:Get("imcd","require_energy",false);
	self.Player = nil;
	self.AdvanceTimer = 0;
	self.AtlSkin = false;
	...
	self.EffDColor = Color(251,136,12,0);
...
function ENT:SpawnFunction(p,t)
	...
	if (IsValid(p)) then
		local PropLimit = GetConVar("CAP_imcd_max"):GetInt()
		if(p:GetCount("CAP_imcd")+1 > PropLimit) then
	...
	local e = ents.Create("industrial_molecular_construction_device");
	e:SetPos(pos);
	e:SetAngles(ang);
	e:DrawShadow(true);
	e:SetVar("Owner",p)
...
function ENT:Think()
    if(self.Create)then
	    ...
	    if(self.InitCreate)then
		    ...
		    self.Entity:EmitSound( "tech/mcd_spawn.wav", 100, 47 );
			if(self.HasWire) then
				self:SetWire("Active", 1)
			end
		else
			local energy2 = 0
			if (StarGate.HasResourceDistribution) and self.RequireEnergy then
				energy2 = self:GetResource("energy",needed_energy);
			else
				energy2 = 999999
			end
			if (energy2 < 1000) then
				self:EmitSound("ambient/energy/powerdown2.wav",100,80)
				self.Create = false;
		    	self.Entity:SetNWInt("EntProgress",0);
				self.Entity:SetNWInt("Progress",0);
		    	self.InitCreate = true;
				...
				if (StarGate.HasResourceDistribution) then
					if (self.Current_class=="zpm_mk3") then
						self:ConsumeResource("energy",4000);
					else
	            ...
	            if(self.Entity:GetNWInt("EntProgress") == 255)then
					...
					util.PrecacheSound("tech/mcd_spawn.wav")
			        self.Entity:EmitSound( "tech/asgard_teleport.mp3", 100, 65 )
...
self.Current_class = class
	...
	if (class=="fnp90" or class=="weapon_zat" or class=="weapon_asura" or class=="sg_medkit" or class=="weapon_gdo" or class=="ancient_obelisk") then realclass = class; class = "prop_physics"; end
    local e = ents.Create(class);
	if (not IsValid(e)) then return end
	self:SetWire("Creation Class",orig_class);
    if (class=="zpm_mk3" and StarGate.CFG:Get("imcd","allow_tzmp",false)) then
    	self.Speed = 1000/600;
    	local rnd = StarGate.CFG:Get("imcd","tzmp_chance",2);
...
net.Receive("MCD_Abort",function(len,ply)
	...
	if (not IsValid(self) or self.Player!=ply or not IsValid(self.Ent)) then return end
...
function ENT:PostEntityPaste(player, Ent,CreatedEntities)
	...
	if (IsValid(player)) then
		player:AddCount("CAP_imcd", self.Entity)
		self.Owner = player;
	end
...
if (StarGate and StarGate.CAP_GmodDuplicator) then
	duplicator.RegisterEntityClass( "industrial_molecular_construction_device", StarGate.CAP_GmodDuplicator, "Data" )
end
