...
if SERVER then
    ...
    function ENT:Think(ply)
        ...
        if self.Active and IsValid(self.Driver) then
            if (math.abs(self.Driver:GetPos().x - self.Stand:GetPos().x) > 100) then
                self.Entity:Exit()

                return
            elseif (math.abs(self.Driver:GetPos().y - self.Stand:GetPos().y) > 100) then
                self.Entity:Exit()

                return
            end

            if (self.Driver:KeyDown(IN_ATTACK) and self.Open and self.NextFire <= CurTime()) then
                self.Entity:FireWeapon()
            end
