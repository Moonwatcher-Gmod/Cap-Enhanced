...
if SERVER then
    ...
    function ENT:SpawnRest(p)
        ...
        if CPPI and IsValid(p) and ent1.CPPISetOwner then
            ent1:CPPISetOwner(p)
        end

        local data = ent1:GetAttachment(ent1:LookupAttachment("Ball"))

        if (not (data and data.Pos)) then
            self:OnRemove()
        ...
        if CPPI and IsValid(p) and ent2.CPPISetOwner then
            ent2:CPPISetOwner(p)
        end
    ...
    function ENT:TriggerInput(variable, value)
        ...
        elseif (variable == "Reload") then
            self.WireReload = value
    ...
    function ENT:Think(ply)
        ...
        if (self.Active and IsValid(self.Driver)) then
            if (self.Driver:KeyDown(IN_USE) and not self.Pressed) then
                self.Entity:Exit()
            ...
            if (self.WireActive == 1) then
                if ((self.WireReload == 1) and (self.Bullets == 0)) then
                    self.Reloadb = true
                end
    ...
    function ENT:PhysicsUpdate(phys, deltatime)
        ...
        if (self.Active and IsValid(self.Driver)) then
            if (self.Control == 1) then
                ...
                if (self.Driver:KeyDown(IN_ATTACK) and self.CanFire) then
                    self.Entity:Shoot()
                end
            else
                if self.Driver:KeyDown(IN_MOVELEFT) then
                    self.Yaw = math.Approach(self.Yaw, 90, 0.5)
                end
                ...
                if self.Driver:KeyDown(IN_FORWARD) then
                    self.Pitch = math.Approach(self.Pitch, 85, 0.5)
                end

                if self.Driver:KeyDown(IN_BACK) then
                    self.Pitch = math.Approach(self.Pitch, 15, 0.5)
                end

                if (self.Driver:KeyDown(IN_JUMP) and self.CanFire) then
                    self.Entity:Shoot()
                end
            end
        else
            if (self.WireActive == 1) then
                ...
                if ((self.WireShoot == 1) and self.CanFire) then
                    self.Entity:Shoot()
                end
    ...
    function ENT:Use(ply, caller)
        if (not self.Active and not IsValid(self.Driver) and not self.Pressed) then
            self.Entity:Enter(ply)
        end
    end
    ...
    function ENT:Shoot()
        ...
        if (self.StargateTrace.HitSky or (mat == MAT_FLESH) or (mat == MAT_METAL) or (mat == MAT_GLASS)) then
            smoke = 0
        end
...
if CLIENT then
    ...
    function ENT:Think()
        if not IsValid(self.Stand) then
            self.Stand = self:GetNetworkedEntity("Stand")
        end

        if not IsValid(self.Turn) then
            self.Turn = self:GetNWEntity("Turn")
        end

        if not IsValid(self.Cann) then
            self.Cann = self:GetNWEntity("Cann")
        end
    ...
    function StatRailGCalcView(Player, Origin, Angles, FieldOfView)
        ...
        if (self and self:IsValid() and self:GetClass() == "stationary_railgun") then
            ...
            if (not (data and data.Pos)) then
                data.Pos = self.Stand:GetPos() + self.Stand:GetUp() * 500
            end
