...
if SERVER then
        ...
        elseif (type == 1) then
            if (self.AutoStop) then
                ...
                if (IsValid(gate) and not gate.IsOpen) then
                    ply:SendLua("GAMEMODE:AddNotify(\"Can't start counter when stargate is closed with auto-reset option!", NOTIFY_ERROR, 5) --\"buttons/button2.wav\" )")
                    surface.PlaySound()
    ...
    function ENT:TriggerInput(variable, value)
        ...
        elseif (variable == "Pause") then
            self.WirePause = util.tobool(value)
        elseif (variable == "Reset") then
            self.WireReset = util.tobool(value)
        elseif (variable == "Minutes") then
            self.WireMinutes = math.Clamp(value, 0, 59)
        elseif (variable == "Seconds") then
            self.WireSeconds = math.Clamp(value, 0, 59)
        elseif (variable == "Auto Start") then
            self.AutoStart = util.tobool(value)
        elseif (variable == "Auto Reset") then
            self.AutoStop = util.tobool(value)
        elseif (variable == "Auto Close") then
            self.AutoClose = util.tobool(value)
        elseif (variable == "Count Up") then
            self.CountUp = util.tobool(value)
            self.WireReset = true
        elseif (variable == "Normal Font") then
            self:SetNWBool("Font", util.tobool(value))
        end
    end
...
if CLIENT then
    ...
    function ENT:Draw()
        ...
        if (Time < 11 and maxtime == 0) then
            Col = Color(225, 50, 50, 255)
        elseif (maxtime ~= 0 and Time > maxtime - 11) then
            Col = Color(225, 50, 50, 255)
        end
    ...
    net.Receive("CAP.DestinyTimer", function(len)
        ...
        autostart:SizeToContents()
        autostart:SetTooltip("Automatic starting counter after stargate opens.")
        ...
        autostop:SetText("Auto-reset")
        autostop:SizeToContents()
        autostop:SetPos(10, 75)
        autostop:SetValue(autocloseval)
        autostop:SizeToContents()
        autostop:SetTooltip("Automatic reseting counter after stargate closes. Uncheck this if you want manualy start/stop counter.")
        ...
        autoclose:SizeToContents()
        autoclose:SetTooltip("Automatic closes stargate when time is ended.")
        ...
        countup:SizeToContents()
        countup:SetTooltip("If checked, then counter will count up.")
        ...
        font:SetText("Normal Font")
        font:SizeToContents()
        font:SetPos(10, 135)
        font:SetValue(autocloseval)
        font:SizeToContents()
        font:SetTooltip("If checked, then will be used normal font.")
        ...
        stopButton:SetText("Stop counter")
        ...
        closeButton:SetText("Close stargate")
