...
properties.Add("AntiPrior.Immunity", {
    MenuLabel = "Immunity Mode",
    Order = -170,
    MenuIcon = "icon16/plugin_link.png",
    Filter = function(self, ent, ply)
        if (not IsValid(ent) or not IsValid(ply) or ent:GetClass() ~= "anti_prior") then return false end
        if (not gamemode.Call("CanProperty", ply, "antipriormodify", ent)) then return false end
    ...
    MenuOpen = function(self, option, ent, tr)
        ...
        local option = submenu:AddOption("Owner and friends", function()
            self:SetImmunity(ent, 0)
        end)
        ...
        local option = submenu:AddOption("Only for owner", function()
            self:SetImmunity(ent, 1)
        end)
        ...
        local option = submenu:AddOption("No one", function()
            self:SetImmunity(ent, -1)
        end)
