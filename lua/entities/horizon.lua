...
if SERVER then
    ...
    function ENT:SpawnFunction(ply, tr)
        ...
        if (ply:GetCount("CAP_horizon") + 1 > PropLimit) then
            ply:SendLua("GAMEMODE:AddNotify(\"Horizon Missiles limit reached!\", NOTIFY_ERROR, 5); surface.PlaySound( \"buttons/button2.wav\" )")
    ...
    function ENT:PostEntityPaste(ply, Ent, CreatedEntities)
        ...
        if (StarGate.NotSpawnable(self:GetClass(), ply)) then
            self.Entity:Remove()
    ...
    function ENT:FireMissile()
        ...
        for i = 1, 10 do
            ...
            if (i > 6) then
                decoy = true
                pos = VectorRand() * 10000
                pos.z = 0
            else
                decoy = false

                if self.WireHitPos[i] then
                    pos = self.WireHitPos[i]
                else
                    pos = VectorRand() * 10000
                    pos.z = 0
                end
    ...
    function ENT:RemoveCovers()
        ...
        for i = 1, 4 do
            ...
            if IsValid(self.Covers[i]) then
                ...
                if (Cover:IsValid()) then
                    Cover:EnableMotion(true)
                end
            end

            timer.Simple(10, function()
                if (IsValid(self) and IsValid(self.Covers[i])) then
                    self.Covers[i]:Remove()
                end
            end)
        end
    end
    ...
    function ENT:TriggerInput(variable, value)
        if (variable == "Fire") then
            self.WireFire = value
        elseif (variable == "Release Covers") then
            self.WireRelease = value
        elseif (variable == "Engine") then
            self.WireEngine = value
        elseif (variable == "HitPos1") then
            self.WireHitPos[1] = value
        elseif (variable == "HitPos2") then
            self.WireHitPos[2] = value
        elseif (variable == "HitPos3") then
            self.WireHitPos[3] = value
        elseif (variable == "HitPos4") then
            self.WireHitPos[4] = value
        elseif (variable == "HitPos5") then
            self.WireHitPos[5] = value
        elseif (variable == "HitPos6") then
            self.WireHitPos[6] = value
        end
    end
