...
local function TextEntryLoseFocus(panel, mcode)
    ...
    if (not pnl.m_bLoseFocusOnClickAway and (panel.ignorefocus or IsValid(panel:GetParent()) and IsValid(panel:GetParent():GetParent()) and panel:GetParent():GetParent().ignorefocus)) then
        pnl:KillFocus()
        panel:RequestFocus()
...
function PANEL:Init()
    ...
    self.DropButton.OnMousePressed = function(button, mcode)
        self:OpenMenu(self.DropButton)
    end
    ...
    self.TextEntry.OnMousePressed = function(button, mcode)
