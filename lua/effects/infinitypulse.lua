...
function EFFECT:Think()
	...
	if self.Relative > 5 then return end
...
function EFFECT:Render()
	...
	if self.Relative < 8 then -- Mostly TetaBonitas
		local eye = EyePos()
		local Distance = eye:Distance(self.StartPos)
		local Pos = self.StartPos+(eye-self.StartPos):GetNormal()*math.sin(self.Relative/2)*1.1*Distance/1.15
