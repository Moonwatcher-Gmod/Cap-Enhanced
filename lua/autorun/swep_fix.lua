...
if SERVER then
    AddCSLuaFile("autorun/swep_fix.lua")
...
if not (meta.__SetNextPrimaryFire) then
    meta.__SetNextPrimaryFire = meta.SetNextPrimaryFire
end

if not (meta.__SetNextSecondaryFire) then
    meta.__SetNextSecondaryFire = meta.SetNextSecondaryFire
end

--################### The actual fix @aVoN
local function PrimaryAttack(self, ...)
    if (self.__PrimaryAttack and (self.__NextPrimaryAttack or 0) < CurTime()) then return self:__PrimaryAttack(...) end
end
...
function meta:SetNextPrimaryFire(delay)
    ...
    if (IsValid(self.Owner) and self.Owner:IsPlayer()) then
        ...
        if ((self.Owner:Ping() + COMPENSATION) / 1000 > delay) then
            delay = delay + COMPENSATION / 1000
        end
