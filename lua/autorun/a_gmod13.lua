...
function file.Open(path, mode, param)
    ...
    if (dir:upper() == "DATA" and not file.Exists(path, dir) and (mode == "r" or mode == "rb")) then
        ...
        if path and (path:lower():find("^expression2/cap_shared/(.*).lua$") or path:lower():find("^starfall/cap_shared/(.*).lua$")) then return file_Open("lua/data/" .. path, mode, "GAME") end
    end
...
function file.Find(path, dir, order)
    ...
    if (dir:upper() == "DATA") then
        ...
        if path and (path:lower():find("^expression2/cap_shared/(.*)$") or path:lower():find("^starfall/cap_shared/(.*)$")) then
