...
function LANGParser:StripQuotes(s)
    ...
    --" <-- needed, to make my NotePad++ to show the functions below
    return s:gsub("^[\"'](.+)[\"']$", "%1")
end
...
function LANGParser:parse()
    ...
    for k, v in pairs(exploded) do
        ...
        -- Only add lines with contents (no commented lines)
        if (line ~= "" and not multiline) then
    ...
    for k, lang in pairs(fo) do
        ...
        if (ini and ini.messages) then
            for _, v in pairs(ini.messages) do
                if (v["global_lang_name"]) then
                    lang_names["Name"][lang] = v["global_lang_name"]
                    lang_names["Lang"][v["global_lang_name"]] = lang
                    break
                end
            end
        end
...
function SGLanguage.SetClientLanguage(lang)
    ...
    timer.Create("SGLanguage.Reload", 0.5, 1, function()
        RunConsoleCommand("sg_language_reload")
    end)
end
...
function SGLanguage.ValidMessage(message)
    if (SGLanguage_Messages[message] and tostring(SGLanguage_Messages[message]) ~= "") then return true end
...
function SGLanguage.CountMessagesInLanguage(lang, return_msgs)
    ...
    if (return_msgs) then return count, msgs end
...
if (CLIENT) then
    function SGLanguage.ReloadSGLanguages(no_msg)
        ...
        if (not no_msg) then
            MsgN("Languages successfully reloded.")
        end
    end

    concommand.Add("sg_language_reload", function()
        SGLanguage.ReloadSGLanguages()
    end)
end
